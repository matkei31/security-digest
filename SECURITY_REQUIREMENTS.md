...
- **Out of scope:** Runtime and platform hardening beyond the accepted BL-025 loader-boundary change, the accepted BL-026 workflow hardening (Action pinning, GitHub Actions Dependabot, and production concurrency), the accepted BL-027 Action major-version upgrade (`actions/checkout` v7.0.1, `actions/setup-python` v7.0.0), and the remainder of BL-009's About/disclaimer/correction-contact/sitemap/robots.txt/canonical/OG/favicon/content-SEO UI (per-source content usage mode enforcement is recorded by the BL-032 implementation described below — BL-032 itself is user-accepted and merged, though this document's own promotion of that material to Approved was never separately completed; see GAP-016; a mandatory Content-Security-Policy remains out of scope, see GAP-018 and section 9)

Version 1.7 is a Draft maintenance update layered on top of Version 1.6 (itself Draft, see below).
It records the BL-034 implementation (Draft, pending user acceptance, branch
`feature/bl034-cloudflare-web-analytics`): adding Cloudflare Web Analytics' manual (non-proxied,
DNS/nameservers unchanged) JavaScript beacon to every generated page (top page, Archive index, and
every daily Archive page) via `render_cloudflare_web_analytics_html()`, together with a short,
non-legally-conclusory analytics-disclosure `<footer>` via `render_analytics_footer_html()`
(`fetch.py`). This is the first external/third-party browser script and first external network
connection the published site has ever made: the script itself loads from
`static.cloudflareinsights.com`, and the beacon separately sends measurement data to
`cloudflareinsights.com` (specifically `cloudflareinsights.com/cdn-cgi/rum`) — these are two
distinct hosts, not one. It is added because the site previously had zero pageview measurement
(see [BL-034](BACKLOG.md#bl-034--閲覧計測基盤)
...
trigger text with the fact that BL-032 is now user-accepted and merged (see section 6.10 and
section 8) — this is a wording correction to match BL-032's already-accepted status, not a new
requirement or a re-evaluation of BL-032 itself. This Version's own Draft status means none of
this is yet approved policy, and it is not a pre-approval of DNS changes, Cloudflare account
operations, Search Console verification, production execution, `workflow_dispatch`, or any
GitHub-side setting change; BL-034's own acceptance plan governs when this ships to
`monomidigest.com`.

Version 1.5 is the most recent **Approved** architecture security-requirements baseline, not
the GitHub vulnerability-reporting policy normally placed in a `SECURITY.md` file. Version 1.6
was a **Draft** maintenance update layered on top of that Approved Version 1.5 baseline,
recording the BL-032 implementation described below as Draft at the time; Version 1.6 was never
itself independently promoted to Approved status, even though BL-032's own runtime implementation
was separately user-accepted and merged afterward (see GAP-016). Version 1.7 (this Version) is a
further Draft maintenance update layered on top of Version 1.6, recording the BL-034
implementation described above. Until the user reviews and accepts the current Draft material,
only Version 1.5 is approved policy, and neither Version 1.6's nor Version 1.7's new or changed
material (see below) is. Version 1.5's own approval is recorded
...
| Third-party browser scripts and client-side analytics | SR-047 | Cloudflare Web Analytics manual (non-proxied) beacon (script loads from `static.cloudflareinsights.com`; measurement data sent to `cloudflareinsights.com`/`cdn-cgi/rum`) on every generated page, plus a footer analytics-disclosure notice (Draft, pending BL-034 user acceptance); no CSP added | [`fetch.py`](fetch.py), [`test_archive.py`](test_archive.py) `Bl034CloudflareWebAnalyticsTest`, `Bl034CheckedInHtmlAuditTest`, [BL-034](BACKLOG.md#bl-034--閲覧計測基盤) | Met (Draft) | Met 1 / Partial 0 / Not met 0 / Unverified 0 |
...
| Mandatory CSP in this Draft | Not approved as a new control here. BL-034 (Version 1.7) deliberately reviewed and declined a full CSP rollout when adding the site's first third-party script (Cloudflare Web Analytics); see GAP-018, SR-047. A future CSP would need `script-src static.cloudflareinsights.com` (where the beacon script itself loads from) and `connect-src cloudflareinsights.com` (where the beacon POSTs measurement data, specifically to `cloudflareinsights.com/cdn-cgi/rum`) — two distinct hosts, not one. Re-evaluate before a second third-party script, forms, or a materially different browser-side risk profile is introduced. |
