...
- **Status:** Approved
- **As of:** 2026-08-03
- **Scope:** Current static GitHub Pages site and its repository-backed generation pipeline; this Version adds the first third-party browser script the site ever loads (BL-034, Cloudflare Web Analytics)
- **Out of scope:** Runtime and platform hardening beyond the accepted BL-025 loader-boundary change, the accepted BL-026 workflow hardening (Action pinning, GitHub Actions Dependabot, and production concurrency), the accepted BL-027 Action major-version upgrade (`actions/checkout` v7.0.1, `actions/setup-python` v7.0.0), and the remainder of BL-009's About/disclaimer/correction-contact/sitemap/robots.txt/canonical/OG/favicon/content-SEO UI (per-source content usage mode enforcement is recorded by the BL-032 implementation described below — BL-032 itself is user-accepted and merged; its current-state material is carried forward and approved as part of this Version; see GAP-016; a mandatory Content-Security-Policy remains out of scope, see GAP-018 and section 9)

Version 1.7 is an **Approved** maintenance update layered on top of Version 1.6 (a historical
Draft, see below). It records the BL-034 implementation (user-accepted, accepted implementation
head `6d032e702e1b118bc6da86b981a4189b4a85e15b`, branch `feature/bl034-cloudflare-web-analytics`):
adding Cloudflare Web Analytics' manual (non-proxied, DNS/nameservers unchanged) JavaScript beacon
to every generated page (top page, Archive index, and every daily Archive page) via
`render_cloudflare_web_analytics_html()`, together with a short, non-legally-conclusory
analytics-disclosure `<footer>` via `render_analytics_footer_html()` (`fetch.py`). This is the
first external/third-party browser script and first external network connection the published
site has ever made: the script itself loads from `static.cloudflareinsights.com`, and the beacon
separately sends measurement data to `cloudflareinsights.com` (specifically
`cloudflareinsights.com/cdn-cgi/rum`) — these are two distinct hosts, not one. It is added because
the site previously had zero pageview measurement (see [BL-034](BACKLOG.md#bl-034--閲覧計測基盤)
and [SD-032](DECISIONS.md#sd-032--adopt-cloudflare-web-analytics-and-google-search-console-for-bl-034)).
This Version adds new SR-047 and new GAP-018 (see below); it does not change the requirements or
historical decisions recorded in SR-001–SR-046 or GAP-001–GAP-017, or any other Version 1.0–1.6
history. Separately from BL-034, and because it had not been done since Version 1.6 was written,
this Version also synchronizes SR-044–SR-046's and GAP-016–GAP-017's current-state, evidence, and
trigger text with the fact that BL-032 is user-accepted and merged (see section 6.10 and
section 8) — this is a wording correction to match BL-032's already-accepted status, not a new
requirement or a re-evaluation of BL-032 itself. This Version's Approved status covers the BL-034
repository implementation described above, following independent review rounds 1 and 2 finding no
remaining Blockers, full unittest 1577 tests OK, `git diff --check` clean, and 0 unresolved review
threads on [PR #72](https://github.com/matkei31/security-digest/pull/72) (see section 12). By the
time this Version's repository implementation was accepted, the user had already registered
`monomidigest.com` as a site/hostname in Cloudflare Web Analytics and retrieved the manual beacon
snippet — that Cloudflare account operation is what supplied the token this implementation embeds;
no DNS, proxy, or nameserver migration to Cloudflare was made or is made by that manual,
non-proxied setup. What remained unconfirmed at that time was publication itself (PR #72 had not
yet merged), and consequently Cloudflare Web Analytics dashboard data receipt and Google Search
Console Domain-property verification, both tracked as BL-034's residual post-merge work; their
being unconfirmed at that time did not block this Version's own approval, which was scoped to the
repository implementation, not to those external, browser-based confirmations. Since then, PR #72 merged, GitHub Pages published the
change, and — per BL-034's closeout record — on 2026-08-03 the user confirmed the Cloudflare Web
Analytics dashboard is receiving data and that Google Search Console verified Domain-property
ownership for `monomidigest.com` (via a DNS record at the domain-name provider; the Google
verification TXT value is not stored in this repository). This later operational confirmation
does not itself re-approve this document or bump its Version — it completes BL-034's evidence
trail (see SR-047, GAP-018, and section 12) without changing this Version's own security
requirements or its CSP/SRI judgments. Production execution, `workflow_dispatch`, and any
GitHub-side setting change beyond ordinary GitHub Pages publication of this Version's own merge
remain separately authorized events.

Version 1.7 is now the most recent **Approved** architecture security-requirements baseline, not
the GitHub vulnerability-reporting policy normally placed in a `SECURITY.md` file. Version 1.5 was
the previous Approved baseline. Version 1.6 was a **Draft** maintenance update layered on top of
that Approved Version 1.5 baseline, recording the BL-032 implementation described below as Draft
at the time; Version 1.6 was never itself independently promoted to Approved status, even though
BL-032's own runtime implementation was separately user-accepted and merged afterward (see
GAP-016) — that current-state material is now carried forward and approved as part of Version 1.7,
rather than through a separate, retroactive promotion of Version 1.6 itself. Version 1.7 (this
Version) is a further maintenance update layered on top of Version 1.6, recording the BL-034
implementation described above; it is Approved, per the acceptance recorded in section 12.
Version 1.5's own approval is recorded
by [SD-030](DECISIONS.md#sd-030--approve-source-usage-policy-version-01-and-defer-runtime-enforcement-to-bl-032).
Version 1.4 retains
...
of any per-source content usage mode; that implementation was registered as
[BL-032](BACKLOG.md#bl-032--取得元別content-usage-policy-enforcement). Version 1.5's approval is
the audit-and-policy approval described above; it was not a pre-approval of BL-032's runtime
enforcement, of production execution, of `workflow_dispatch`, or of any GitHub-side setting
change.

Version 1.6 is a Draft maintenance update. It records the BL-032 implementation (Draft, pending
user acceptance, branch `feature/bl032-content-usage-enforcement`): per-source `content_usage_mode`
enforcement in `source_definitions.json` and `fetch.py`/`daily_json.py` across collection, Gemini
input, storage, daily JSON, Today's Brief/dashboard aggregation, and HTML rendering; the Gemini
`paid_verified` data-use gate now enforced at runtime (not merely recorded); the common
feed-native rich-content path from [SD-002](DECISIONS.md#sd-002--use-feed-native-rich-content-without-additional-article-http-requests)
replaced by a per-source policy gate (all 17 sources currently have `allow_rich_content=false`);
output-length and verbatim-long-match validation with automatic downgrade to a metadata-only
equivalent on violation; and daily JSON `SCHEMA_VERSION` 2 (policy-excluded articles separated
from AI-evaluated ones in `run`/`counts`, with past schema-version-1 daily JSON read unchanged
under the preserved legacy contract). This Version updates SR-044 to `Met` and GAP-016 to
`Implemented` accordingly (see below); it does not change SR-045/SR-046, GAP-017, or any
Version 1.0–1.5 Approved history. This Version's own Draft status means none of this is yet
approved policy, and it is not a pre-approval of production execution, `workflow_dispatch`, real
Gemini API calls, or any GitHub-side setting change; BL-032's own acceptance plan governs when
(and whether) this enforcement runs against real production traffic.
...
- a documented content-usage policy for external sources
  ...
  audit-only; per-source enforcement in production code is implemented by BL-032 (user-accepted
  and merged; see GAP-016).
...
| Source-terms audit and content-usage policy | Trusted configuration / public | [SOURCE_USAGE_POLICY.md](SOURCE_USAGE_POLICY.md) Approved 0.1 | Public and versioned; records per-source official terms, confidence, and unresolved issues; enforcement in production code is implemented and live via BL-032, which is user-accepted and merged (see GAP-016; this document's own Approved promotion of that record was completed by Version 1.7 — that current-state material is carried forward and covered by Version 1.7's Approved status, not an unresolved documentation step) |
...
| ID | Requirement | Rationale | Current state | Evidence | Gap / exception | Re-evaluation trigger |
|---|---|---|---|---|---|---|
| SR-044 | Audit each configured source's official terms, license, or FAQ, and record a proposed content usage mode (`structured_open`, `feed_summary`, `limited_feed_analysis`, `metadata_only`, `disabled_legal_review`), confidence, unresolved issues, and a recheck trigger before assuming broader reuse than description/metadata is permitted; enforce the recorded mode in production. | Feed availability does not by itself authorize AI processing, public summarization, or content storage. | Met | [SOURCE_USAGE_POLICY.md](SOURCE_USAGE_POLICY.md) Approved 0.1 records the audit for all 17 configured sources, including official evidence URLs/types, confidence, and recheck triggers. `limited_feed_analysis` (`the_hacker_news`, `krebs_on_security`) is an explicit, bounded risk acceptance, not a determination that reuse is permitted. [BL-032](BACKLOG.md#bl-032--取得元別content-usage-policy-enforcement) (user-accepted and merged, [PR #69](https://github.com/matkei31/security-digest/pull/69)) adds a `policy` object with `content_usage_mode` to every `source_definitions.json` entry, fail-closed loader validation, and enforcement in `fetch.py`/`daily_json.py` across collection, Gemini input, storage, and public-summary paths; [`test_content_usage_policy.py`](test_content_usage_policy.py). | BL-032's implementation is accepted and merged; this document's own Version 1.6 record of it was not separately re-promoted to Approved (a documentation-maintenance gap, not an implementation gap — see GAP-016). | Any source-terms, license, or FAQ change; addition of a new source. |
| SR-045 | Do not send publisher-derived article content to the Gemini API for AI processing or public summarization unless the Gemini API's data-use terms for the request path (`paid_verified` Cloud Billing-linked Project) have been owner-confirmed; treat `unpaid` or `unknown` status as requiring metadata-only handling for sources whose own terms do not clearly authorize AI processing. | Google's Gemini API terms treat Unpaid Services differently from Paid Services with respect to using submitted content for product/model improvement. | Met | [SOURCE_USAGE_POLICY.md](SOURCE_USAGE_POLICY.md) section 5 (Gemini data-use gate) records `gemini_data_use_status: paid_verified`, owner-confirmed 2026-07-29 via the Google AI Studio API Keys screen (the `security-digest` Google Cloud Project has active Cloud Billing, Tier 1 pay-as-you-go); no API key, Project ID, billing account ID, amount, or screenshot is recorded. | The Gemini-side data-use gate condition is satisfied, and this `paid_verified` gate is enforced at runtime. Per-source production enforcement of content usage modes is implemented, user-accepted, and merged via BL-032 (see SR-044, GAP-016), and each source's own terms conditions (e.g. `google_tag`/`mandiant`'s classification under the Google Terms version confirmed effective 2026-07-30, see [SOURCE_USAGE_POLICY.md](SOURCE_USAGE_POLICY.md) section 8) continue to be governed by [SOURCE_USAGE_POLICY.md](SOURCE_USAGE_POLICY.md) and SR-044/GAP-016, unaffected by this confirmation. See GAP-017. | Owner confirms a billing/Project/API-key change; Gemini API terms change. |
| SR-046 | Track each disabled source's re-enablement condition (`activation_condition`) explicitly, including any robots.txt, User-Agent-scope, or written-permission condition, and do not re-enable a source until its recorded condition is satisfied. | Ad hoc re-enablement without re-checking terms would undo the audit's purpose. | Partially met | [`source_definitions.json`](source_definitions.json): non-empty `activation_condition` for `cisa`, `crowdstrike`, `cloudflare`, and `dark_reading`; [`test_source_definitions.py`](test_source_definitions.py) | `nist_nvd` is disabled but its `activation_condition` field is an empty string; only its `notes` field records why standalone collection is off (a historical, pre-BL-030 note), not a re-enablement condition in the `activation_condition` field this requirement tracks. This Version does not add a `nist_nvd.activation_condition` value, since doing so would change a `source_definitions.json` field for a source other than Dark Reading, which is outside this Version's scope. | A future ticket that reconsiders standalone `nist_nvd` article collection, or otherwise records its `activation_condition`. |

### 6.11 Third-party browser scripts and client-side analytics

| ID | Requirement | Rationale | Current state | Evidence | Gap / exception | Re-evaluation trigger |
|---|---|---|---|---|---|---|
| SR-047 | Any third-party browser script must be added deliberately, scoped to one documented purpose, use the vendor's least-data configuration reasonably available (prefer a cookie/localStorage-free option when one meets the need), and be disclosed to readers (service name, purpose, what is collected, and the external destination(s)) without asserting a legal conclusion the repository cannot verify. | External scripts add a new network destination and third-party code execution in the reader's browser; readers should be able to know what runs and why, and a bare technical claim ("no cookies") is not the same as a legal compliance determination. | Met | [`fetch.py`](fetch.py): `render_cloudflare_web_analytics_html()` emits exactly the Cloudflare-issued manual (non-proxied) beacon snippet — script loads from `static.cloudflareinsights.com`, one script per page, no other script tag; `render_analytics_footer_html()` emits a `<footer>` disclosure naming Cloudflare Web Analytics, its stated purpose, Cloudflare's own no-cookie/no-localStorage claim (phrased as a quoted vendor description, not a legal assertion), the aggregate data collected, the script's read source (`static.cloudflareinsights.com`), and the measurement-data destination (`cloudflareinsights.com`, specifically `cloudflareinsights.com/cdn-cgi/rum`) — these two hosts are named separately, not conflated into one; [`test_archive.py`](test_archive.py) `Bl034CloudflareWebAnalyticsTest`, `Bl034CheckedInHtmlAuditTest`; [`test_fetch.py`](test_fetch.py) `test_only_the_documented_cloudflare_beacon_script_is_emitted`, `test_no_html_comment_carries_brief_content`. User-accepted at [PR #72](https://github.com/matkei31/security-digest/pull/72) head `6d032e702e1b118bc6da86b981a4189b4a85e15b`. | The DNS registrar/manager (XServer) and nameservers are unchanged; Cloudflare's manual/non-proxied beacon embed method itself makes no DNS record change and no proxy migration. Separately, as part of Google Search Console Domain-property verification (BL-034 closeout, 2026-08-03), a new Google verification TXT record was added at XServer DNS — no existing DNS record was deleted or replaced, and the TXT value itself is not stored in this repository. No Subresource Integrity is applied to the beacon, because Cloudflare's manual-embed beacon has no version-pinned URL to hash against per Cloudflare's own FAQ; no Content-Security-Policy is added at this Version (see GAP-018 and section 9); GA4, Umami, Plausible, and Google Tag Manager were not adopted in this Version; public deployment to `monomidigest.com`, Cloudflare Web Analytics dashboard data receipt, and Google Search Console Domain-property ownership verification for `monomidigest.com` were all confirmed as part of BL-034 closeout — see GAP-018. The SRI/CSP/third-party-vendor-risk judgments above are unchanged by that confirmation. | Adding a second third-party script; replacing or upgrading the analytics vendor; adding custom events or campaign (UTM) tracking; Cloudflare offering a version-pinned/SRI-compatible manual-embed URL. |
...
| Source content-usage policy and AI provider data-use boundary | SR-044–SR-046 | Read-only official-terms audit of all 17 sources with proposed content usage modes ([SOURCE_USAGE_POLICY.md](SOURCE_USAGE_POLICY.md) Approved 0.1), enforced at runtime by BL-032 (user-accepted and merged; this document's own Approved promotion of that record was completed by Version 1.7, consistent with GAP-016); owner-confirmed Gemini Paid Services gate (`paid_verified`), enforced at runtime; documented per-source `activation_condition` for disabled sources, except `nist_nvd` (empty `activation_condition`) | [SOURCE_USAGE_POLICY.md](SOURCE_USAGE_POLICY.md), [`source_definitions.json`](source_definitions.json), [`test_source_definitions.py`](test_source_definitions.py), [`test_content_usage_policy.py`](test_content_usage_policy.py) | Partially met | Met 2 / Partial 1 / Not met 0 / Unverified 0 |
...
| Third-party browser scripts and client-side analytics | SR-047 | Cloudflare Web Analytics manual (non-proxied) beacon (script loads from `static.cloudflareinsights.com`; measurement data sent to `cloudflareinsights.com`/`cdn-cgi/rum`) on every generated page, plus a footer analytics-disclosure notice (BL-034 `完了`; public deployment, Cloudflare dashboard receipt, and Search Console Domain-property verification all confirmed 2026-08-03); no CSP added | [`fetch.py`](fetch.py), [`test_archive.py`](test_archive.py) `Bl034CloudflareWebAnalyticsTest`, `Bl034CheckedInHtmlAuditTest`, [BL-034](BACKLOG.md#bl-034--閲覧計測基盤) | Met | Met 1 / Partial 0 / Not met 0 / Unverified 0 |
...
| GAP-016 | Security gap | Implemented | SR-044 | [SOURCE_USAGE_POLICY.md](SOURCE_USAGE_POLICY.md) Approved 0.1 assigns a content usage mode (including the `limited_feed_analysis` risk-acceptance mode) to each of the 17 sources; [BL-032](BACKLOG.md#bl-032--取得元別content-usage-policy-enforcement) adds a `policy` object with `content_usage_mode` to every `source_definitions.json` entry and enforces it in `fetch.py`/`daily_json.py`: collection annotates each item with `source_id`/`content_policy`, `enrich_with_ai()` skips Gemini entirely for `ai_eligible=false` items and gates rich content by `allow_rich_content` (false for all 17 sources, replacing [SD-002](DECISIONS.md#sd-002--use-feed-native-rich-content-without-additional-article-http-requests)'s common path), output-length/verbatim-match violations downgrade to metadata-only, and daily JSON schema v2 separates policy-excluded articles from AI-evaluated ones. | Before BL-032, a `feed_summary`, `limited_feed_analysis`, or `metadata_only` source was processed the same as a `structured_open` source in production code; that gap is closed. | BL-032's implementation is proportionate to the audit's proposed restrictions; concrete thresholds (output-length caps, verbatim-match minimum length, transient input cap) are recorded once in `daily_json.py` and documented, not invented ad hoc. | [BL-032](BACKLOG.md#bl-032--取得元別content-usage-policy-enforcement) is user-accepted and merged ([PR #69](https://github.com/matkei31/security-digest/pull/69)); its runtime enforcement is live in production, not Draft. This SECURITY_REQUIREMENTS.md document's own record of that fact was originally written as part of Version 1.6, Draft, and was not separately re-promoted to Approved at the time; that documentation-maintenance gap is now resolved, since this current-state material is carried forward and covered by Version 1.7's own Approved status (see section 12) rather than through a separate, retroactive promotion of Version 1.6 itself; see the intro's Version 1.6/1.7 history for what each Version actually recorded at the time. | [BL-032](BACKLOG.md#bl-032--取得元別content-usage-policy-enforcement); [`test_content_usage_policy.py`](test_content_usage_policy.py) | Any confirmed production processing of a `feed_summary`/`limited_feed_analysis`/`metadata_only` source beyond its recorded mode. |
| GAP-017 | Owner verification | Completed owner verification | SR-045 | `gemini_data_use_status` was `unknown`; the repository owner has confirmed, via the Google AI Studio API Keys screen on 2026-07-29, that the `security-digest` Google Cloud Project used for the Gemini API key has active Cloud Billing (Tier 1, pay-as-you-go). | Resolved for the Gemini-side data-use question: submitted content is not subject to Unpaid Services product/model-improvement use for this Project, as owner-verified. | Owner confirmation was proportionate: a non-confidential yes/no on active billing, without recording API keys, billing amounts, account IDs, or screenshots. | Confirmed 2026-07-29: `security-digest` Project, active Cloud Billing, Tier 1 pay-as-you-go. This confirms the Gemini-side condition only; per-source production enforcement of content usage modes and each source's own terms conditions are unaffected and remain governed by SR-044/GAP-016 and [SOURCE_USAGE_POLICY.md](SOURCE_USAGE_POLICY.md) respectively. | [SOURCE_USAGE_POLICY.md](SOURCE_USAGE_POLICY.md) section 5; [BL-032](BACKLOG.md#bl-032--取得元別content-usage-policy-enforcement) (user-accepted and merged, [PR #69](https://github.com/matkei31/security-digest/pull/69); runtime enforcement live) | Billing cancellation, Project change, or API key migration; re-verify if any occurs. |
| GAP-018 | Policy decision | Implemented | SR-047 | This is the first third-party remote script the published site has ever loaded (Cloudflare Web Analytics' manual, non-proxied beacon); the manual-embed method offers no version-pinned URL, so no Subresource Integrity can be applied to it (per Cloudflare's own FAQ); and this Version deliberately does not adopt a Content-Security-Policy (see section 9). The absence of prior measurement itself is not treated as a security gap — it is the reason BL-034 exists, not a defect. | A vendor-delivered script can change or be compromised upstream between the repository's own review cycles, and the reader's browser now makes an outbound network connection to an endpoint outside repository control (script from `static.cloudflareinsights.com`, measurement data to `cloudflareinsights.com`/`cdn-cgi/rum`). | Limited to a single, lightweight, vendor-documented analytics script; the page's core rendering does not depend on the script succeeding (loaded via a non-blocking `<script type='module'>`, no other page functionality references it); re-evaluate before adding another script, advertising, forms, or more detailed/campaign tracking rather than adopting CSP or SRI preemptively. | [BL-034](BACKLOG.md#bl-034--閲覧計測基盤) (`完了`, branch `feature/bl034-cloudflare-web-analytics`, accepted implementation head `6d032e702e1b118bc6da86b981a4189b4a85e15b`, merged as [PR #72](https://github.com/matkei31/security-digest/pull/72) merge commit `8cd98e52bfe6164bffa8e10cdbf708eef76d43a1`); [`fetch.py`](fetch.py); [`test_archive.py`](test_archive.py) `Bl034CloudflareWebAnalyticsTest`, `Bl034CheckedInHtmlAuditTest`; [SD-032](DECISIONS.md#sd-032--adopt-cloudflare-web-analytics-and-google-search-console-for-bl-034). | BL-034 is complete: PR #72 merged, GitHub Pages published the change, and on 2026-08-03 the user confirmed both the Cloudflare Web Analytics dashboard is receiving data and Google Search Console Domain-property ownership verification succeeded for `monomidigest.com` (via a DNS record at the domain-name provider; the Google verification TXT value is not stored in this repository). The SRI/CSP/third-party-vendor-risk judgments in this row are unchanged by that confirmation. | Any additional third-party script, advertising, forms, analytics vendor change, or introduction of custom events/campaign tracking beyond what this Version records. |
...
| Mandatory CSP in this Version | Not approved as a new control here. BL-034 (Version 1.7) deliberately reviewed and declined a full CSP rollout when adding the site's first third-party script (Cloudflare Web Analytics); see GAP-018, SR-047. A future CSP would need `script-src static.cloudflareinsights.com` (where the beacon script itself loads from) and `connect-src cloudflareinsights.com` (where the beacon POSTs measurement data, specifically to `cloudflareinsights.com/cdn-cgi/rum`) — two distinct hosts, not one. Re-evaluate before a second third-party script, forms, or a materially different browser-side risk profile is introduced. |
...
BL-034 (Version 1.7) is the review triggered by "adding analytics, tracking, advertising, or a
third-party browser script" above, scoped specifically to adding Cloudflare Web Analytics. That
bullet remains in force for any future analytics/tracking/advertising/script addition beyond what
BL-034 records (see SR-047, GAP-018).
...
- implement per-source `content_usage_mode` enforcement in `source_definitions.json` and
  `fetch.py`/`daily_json.py` (collection, Gemini input, storage, daily JSON, Today's Brief/
  dashboard aggregation, HTML rendering), enforce the Gemini `paid_verified` gate at runtime,
  replace [SD-002](DECISIONS.md#sd-002--use-feed-native-rich-content-without-additional-article-http-requests)'s
  common rich-content path with a per-source policy gate, add output-length/verbatim-match
  validation with automatic metadata-only downgrade on violation, and bump daily JSON
  `SCHEMA_VERSION` to 2 while preserving the legacy schema-version-1 contract for past daily
  JSON unchanged — all on branch `feature/bl032-content-usage-enforcement`, pending user
  acceptance; this bullet does not itself approve that implementation.
...
- **Version 1.6 is a Draft, not an approved maintenance update.** It records the BL-032
  implementation (branch `feature/bl032-content-usage-enforcement`): per-source
  `content_usage_mode` enforcement across collection, Gemini input, storage, daily JSON,
  Today's Brief/dashboard aggregation, and HTML rendering; the Gemini `paid_verified` gate
  enforced at runtime; the common feed-native rich-content path replaced by a per-source policy
  gate; output-length/verbatim-match validation with automatic metadata-only downgrade; and
  daily JSON `SCHEMA_VERSION` 2 with the legacy schema-version-1 contract preserved for past
  daily JSON. This Version updates SR-044 to `Met` and GAP-016 to `Implemented (Draft, pending
  user acceptance)`; it does not change SR-045/SR-046, GAP-017, or the Version 1.0–1.5 Approved
  history. This Version does not itself carry a stable, approved SD entry; it remains Draft
  until the user reviews and accepts it, at which point a future version will record that
  approval (as Version 1.0 through 1.5 each did for their own scope) without reopening SD-024,
  SD-025, SD-028, SD-029, or SD-030. This Version's Draft status is not a pre-approval of
  production execution, `workflow_dispatch`, real Gemini API calls, or any GitHub-side setting
  change.
- **Version 1.7 is an Approved maintenance update, recording the BL-034 implementation described
  above.** The user reviewed and accepted BL-034's repository implementation with this message,
  after independent review rounds 1 and 2 on
  [PR #72](https://github.com/matkei31/security-digest/pull/72) found no remaining Blockers:
  accepted implementation head `6d032e702e1b118bc6da86b981a4189b4a85e15b`, full unittest 1577
  tests OK, `git diff --check` clean, changed files 35, unresolved review threads 0, and Pull
  Request CI run [30765873879](https://github.com/matkei31/security-digest/actions/runs/30765873879)
  success. This approval covers SR-047 (`Met`), GAP-018 (`Policy decision`, `Implemented`), and
  the SR-044–SR-046/GAP-016–GAP-017 current-state wording synchronization with BL-032's
  already-accepted status. At the time of this acceptance, it did not itself confirm that the
  Cloudflare Web Analytics dashboard was receiving data or that Google Search Console
  Domain-property verification had succeeded — both were then unconfirmed, tracked as BL-034's
  residual post-merge work, and did not block this approval, which was scoped to the repository
  implementation. This acceptance does not reopen SD-024, SD-025, SD-028, SD-029, or SD-030, and
  it does not approve any DNS change, Cloudflare account operation, Search Console operation,
  production execution, or `workflow_dispatch`.
- **BL-034 closeout (2026-08-03, same day as the acceptance above).** [PR #72](https://github.com/matkei31/security-digest/pull/72)
  merged as merge commit `8cd98e52bfe6164bffa8e10cdbf708eef76d43a1`, and the merge-triggered
  GitHub Pages deployment succeeded. The user then confirmed, in the browser, that the Cloudflare
  Web Analytics dashboard is receiving data and that Google Search Console verified
  Domain-property ownership for `monomidigest.com` (via a DNS record at the domain-name provider;
  the Google verification TXT value is not stored in this repository). BL-034 is now `完了`
  (see BACKLOG.md). This closeout record does not re-approve Version 1.7 or bump its Version — it
  is the same Approved Version 1.7 acceptance above, with its evidence trail completed; the
  requirements, CSP/SRI judgments, and approval scope recorded in the bullet above are unchanged.
