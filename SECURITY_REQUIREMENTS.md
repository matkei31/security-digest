...
destination (`static.cloudflareinsights.com`) the published site has ever loaded; it is added
because the site previously had zero pageview measurement (see [BL-034](BACKLOG.md#bl-034--閲覧計測基盤)
...
| Source content-usage policy and AI provider data-use boundary | SR-044–SR-046 | Read-only official-terms audit of all 17 sources with proposed content usage modes ([SOURCE_USAGE_POLICY.md](SOURCE_USAGE_POLICY.md) Approved 0.1), now enforced at runtime by BL-032 (Draft); owner-confirmed Gemini Paid Services gate (`paid_verified`), now enforced at runtime; documented per-source `activation_condition` for disabled sources, except `nist_nvd` (empty `activation_condition`) | [SOURCE_USAGE_POLICY.md](SOURCE_USAGE_POLICY.md), [`source_definitions.json`](source_definitions.json), [`test_source_definitions.py`](test_source_definitions.py), [`test_content_usage_policy.py`](test_content_usage_policy.py) | Partially met | Met 2 / Partial 1 / Not met 0 / Unverified 0 |
...
| Third-party browser scripts and client-side analytics | SR-047 | Cloudflare Web Analytics manual (non-proxied) beacon on every generated page, plus a footer analytics-disclosure notice (Draft, pending BL-034 user acceptance); no CSP added | [`fetch.py`](fetch.py), [`test_archive.py`](test_archive.py) `Bl034CloudflareWebAnalyticsTest`, [BL-034](BACKLOG.md#bl-034--閲覧計測基盤) | Met (Draft) | Met 1 / Partial 0 / Not met 0 / Unverified 0 |
...
| GAP-016 | Security gap | Implemented (Draft, pending user acceptance) | SR-044 | [SOURCE_USAGE_POLICY.md](SOURCE_USAGE_POLICY.md) Approved 0.1 assigns a content usage mode (including the `limited_feed_analysis` risk-acceptance mode) to each of the 17 sources; [BL-032](BACKLOG.md#bl-032--取得元別content-usage-policy-enforcement) (Draft) adds a `policy` object with `content_usage_mode` to every `source_definitions.json` entry and enforces it in `fetch.py`/`daily_json.py`: collection annotates each item with `source_id`/`content_policy`, `enrich_with_ai()` skips Gemini entirely for `ai_eligible=false` items and gates rich content by `allow_rich_content` (false for all 17 sources, replacing [SD-002](DECISIONS.md#sd-002--use-feed-native-rich-content-without-additional-article-http-requests)'s common path), output-length/verbatim-match violations downgrade to metadata-only, and daily JSON schema v2 separates policy-excluded articles from AI-evaluated ones. | Before this Version, a `feed_summary`, `limited_feed_analysis`, or `metadata_only` source was processed the same as a `structured_open` source in production code; that gap is now closed by this Version's Draft implementation, pending user acceptance. | This Version's implementation is proportionate to the audit's proposed restrictions; concrete thresholds (output-length caps, verbatim-match minimum length, transient input cap) are recorded once in `daily_json.py` and documented, not invented ad hoc. | Keep this disposition until the user accepts BL-032 (or requests rework); do not treat this Draft implementation as an approved control until acceptance is recorded. | [BL-032](BACKLOG.md#bl-032--取得元別content-usage-policy-enforcement) (Draft, branch `feature/bl032-content-usage-enforcement`); [`test_content_usage_policy.py`](test_content_usage_policy.py) | BL-032 user acceptance (or rejection requiring rework); any confirmed production processing of a `feed_summary`/`limited_feed_analysis`/`metadata_only` source beyond its recorded mode after acceptance. |
...
| GAP-018 | Security gap | Implemented (Draft, pending user acceptance) | SR-047 | Before this Version, the published site loaded zero third-party browser scripts, had no client-side pageview measurement, and had no analytics-disclosure surface for readers; this made establishing a before/after baseline for BL-009 (SEO) impossible. | Without any measurement, SEO/reach decisions had no baseline; without a disclosure, a reader had no way to learn what a future script would do even if one were added carefully. | A single, vendor-documented, cookie/localStorage-free (per Cloudflare's own claim) analytics beacon plus a short footer disclosure is proportionate for a solo-maintained static site's first analytics addition; a full Content-Security-Policy, GA4, Umami, and UTM/custom-event tracking were considered and deliberately deferred (see section 9) rather than adopted preemptively. | [BL-034](BACKLOG.md#bl-034--閲覧計測基盤) (Draft, branch `feature/bl034-cloudflare-web-analytics`); [`fetch.py`](fetch.py); [`test_archive.py`](test_archive.py) `Bl034CloudflareWebAnalyticsTest`; [SD-032](DECISIONS.md#sd-032--adopt-cloudflare-web-analytics-and-google-search-console-for-bl-034). | Keep this disposition until the user accepts BL-034 (or requests rework); do not treat this Draft implementation as an approved control until acceptance is recorded. | BL-034 user acceptance (or rejection requiring rework); any additional third-party script, analytics vendor change, or introduction of custom events/campaign tracking beyond what this Version records. |
...
| Mandatory CSP in this Draft | Not approved as a new control here. BL-034 (Version 1.7) deliberately reviewed and declined a full CSP rollout when adding the site's first third-party script (Cloudflare Web Analytics); see GAP-018, SR-047. A future CSP would need `script-src`/`connect-src` allowances for `static.cloudflareinsights.com`/`cloudflareinsights.com`. Re-evaluate before a second third-party script, forms, or a materially different browser-side risk profile is introduced. |
