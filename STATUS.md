...
- [BL-020](BACKLOG.md#bl-020--収集元一覧の取得元別カラーを廃止する): the per-source color and pill-like treatment has been replaced locally with the UI_SPEC Version 1.3 achromatic plain-text list while preserving the completed 15-source count, enabled set, and definition order. BL-020 is implemented in [Draft PR #41](https://github.com/matkei31/security-digest/pull/41) and awaits user visual acceptance; production remains unchanged.
...
[BL-020](BACKLOG.md#bl-020--収集元一覧の取得元別カラーを廃止する) is implemented in [Draft PR #41](https://github.com/matkei31/security-digest/pull/41) and awaits user visual acceptance, so it is not ranked as an implementation candidate.
