...
- [BL-027](BACKLOG.md#bl-027--github-actions-checkoutsetup-pythonをv7系へmajor-upgradeする) GitHub Actions checkout／setup-python combined major upgrade to v7 — implementation merged; ordinary schedule validation pending. `actions/checkout` pinned to v7.0.1 (`3d3c42e5aac5ba805825da76410c181273ba90b1`) and `actions/setup-python` pinned to v7.0.0 (`5fda3b95a4ea91299a34e894583c3862153e4b97`) at the same full commit SHAs in both `.github/workflows/fetch.yml` and `.github/workflows/pr-ci.yml`, with matching exact-SHA contract tests. [PR #54](https://github.com/matkei31/security-digest/pull/54) final head `241e7f69c9c843fc212c1c590f3a328da5946579` passed [Pull Request CI run 30144069410](https://github.com/matkei31/security-digest/actions/runs/30144069410) — validating hosted-runner/Action-startup compatibility for both Actions — and merged as `69f7da859e1856beffac9fa381f0f0cc92564e36`; the automatic [Pages deployment run 30144096081](https://github.com/matkei31/security-digest/actions/runs/30144096081) succeeded. Dependabot PRs [#51](https://github.com/matkei31/security-digest/pull/51) and [#52](https://github.com/matkei31/security-digest/pull/52) were closed as superseded (plain close with an explanatory comment, no ignore rule). Production `git commit`/`git push` under v7 is **not yet validated**; that happens via the next ordinary schedule run — `workflow_dispatch` is not used for this. No production run or `workflow_dispatch` has occurred. [SECURITY_REQUIREMENTS.md](SECURITY_REQUIREMENTS.md) remains Version 1.3; ARTICLE／BRIEF contracts, daily schema, runtime, `data/`, and `docs/` are unchanged by the BL-027 implementation. BL-027 remains open.
