...
旧daily JSONの表示互換では保存値を書き換えず、日別Archiveの表示時だけ同じ現行形式へ変換または補完する。保存済みの`掲載N件｜…`(pre-BL-048)および`本日の状態（掲載N件）：…`(BL-016以前)は、表示時に件数を保ったまま現行形式へ正規化する。いずれの形式にも一致しない自由文overviewは従来どおりfail-openで全文表示する。
