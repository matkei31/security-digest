...
- 現行UIへAI利用を明示する専用注記は追加しない。記事カード単位・分析区分単位の注記も採用しない。将来Aboutや公開導線を別スコープで設計する場合は、その時点で説明の必要性を改めて裁定する。（[SD-016](DECISIONS.md#sd-016--resolve-the-remaining-bl-004-ui-choices-without-changing-the-accepted-layout)、Version 1.0で確定。Version 1.7 Draftは、下記のとおりこの方針の一部に対する限定例外を提案する。この提案はユーザー目視受入とSD-033の追加によって確定するまでは仕様・Decision上は受入待ちのDraftである。）
  - **維持する方針(このDraftでも変更しない):** サイト全体へgenericな「AIを利用しています」badgeやalertを追加しない。全記事へsource policyと無関係な一律AI noteを追加しない。AI analysisの各見出し・各段落へ反復的なAI labelを追加しない。About／footer等の一般的AI説明はBL-036のscope外。AIであることを過剰に強調し、記事の可読性を損なわない。
  - **限定例外として提案する現行実装(BL-032で実装済み、BL-036のruntime変更はCSSのみ):** [BL-031](BACKLOG.md#bl-031--全取得元の公式規約監査とセキュリティ文書整合化)／[BL-032](BACKLOG.md#bl-032--取得元別content-usage-policy-enforcement)のsource usage policyで必要とされる、source別・content usage mode別のattribution note（`.article-attribution`）は、記事カードへの表示としてBL-032で既に実装・受入・merge済みであり、BL-036が新たに追加したruntime要素はCSSだけである。これはsource policy、利用条件、attribution、bounded AI analysisであること、原文代替ではないこと等を読者へ示すための限定的な注記であり、一律のgeneric AI badgeとは目的・表示条件・文言が異なる。source policy上不要な記事へ推測で表示しない。DOM位置はAI analysisの後、元記事CTAの前を維持する。見た目は本文より低強調であるが、判読可能性を維持する（現行値は3.2参照）。attribution文言と表示条件の正本は[SOURCE_USAGE_POLICY.md](SOURCE_USAGE_POLICY.md)、`source_definitions.json`の`policy`、`render_source_attribution_html()`である。UI_SPECは表示階層と見た目だけを規定し、文言・表示条件の正本を持たない。**Version 1.7 Draftは、この既に実装済みの表示を[SD-016](DECISIONS.md#sd-016--resolve-the-remaining-bl-004-ui-choices-without-changing-the-accepted-layout)の「記事カード単位のAI-use noteを採用しない」という部分に対する限定例外として正式化することを提案する。この限定例外は、ユーザーがPC 1280px／390pxのreview screenshotsを目視受入し、新規SD-033がSD-016のAI-use note条項だけを限定的にsupersedeするまでは確定しない。SD-016のgeneric AI disclosure禁止と、AI-use note以外の6項目は、この提案によって変更しない。**
...
| `.article-attribution` | font-size `10px`、color `#768496`、line-height `1.6`、margin-top `10px`。background／border／border-radius／pillなし。内部link color `#8b949e`、hoverで`#79c0ff`、underline。PC／390px共通(BL-036, Version 1.7 Draft) |
...
11. Version 1.7はBL-036（Fable 5レビューR-01）に基づき、BL-032で既に実装・稼働していた記事カードの`.article-attribution`(source policy別のattribution note)へ、低強調のCSS(font-size 10px、color `#768496`、background／border／pillなし)を追加する。BL-036がruntime側に加えた変更はこのCSSだけであり、attribution文言・mode分岐・DOM順序（AI analysisの後、元記事CTAの前）・HTML escape・URL安全性は変更していない。3.1のAI-use note原則を、サイト全体への一律generic AI note禁止（このDraftでも維持する方針）と、source usage policyが要求するsource別・mode別attribution（BL-032実装済みの表示を[SD-016](DECISIONS.md#sd-016--resolve-the-remaining-bl-004-ui-choices-without-changing-the-accepted-layout)の「記事カード単位のAI-use noteを採用しない」という部分に対する限定例外として正式化する提案）とに区別して明示する。この限定例外の提案は、ユーザーがPC 1280px／390pxのローカルreview screenshotsを目視受入し、[BL-036](BACKLOG.md#bl-036--記事カードのsource-attribution注記を低強調表示へ整える)の受入記録commitで新規SD-033がSD-016のAI-use note条項だけを限定的にsupersedeするまでは確定しない（SD-016のgeneric AI disclosure禁止と他の6項目は対象外であり、この提案によって変更しない）。Version 1.7は2026-08-03時点でDraftであり、上記のユーザー目視受入とSD-033追加までApprovedにしない。
...
| 1.7 | Draft | BL-036(Fable 5レビューR-01)でBL-032実装済みの`.article-attribution`へ低強調CSSを追加(runtime変更はCSSのみ)。AI-use note原則(3.1)のうち一律generic note禁止は維持し、BL-032実装済みのsource-policy-required attributionをSD-016の一部への限定例外として正式化することを提案。この提案はユーザーのPC 1280px／390px目視受入とSD-033追加まで受入待ち |
