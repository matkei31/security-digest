...
- **バージョン:** 1.12
- **状態:** 承認済み
- **最終受入日:** 2026-08-16（Version 1.12）。Version 1.11・1.10は2026-08-15、Version 1.9・1.8は2026-08-14、Version 1.7は2026-08-04である
...
状態行はBL-016で確定し、BL-048で先頭segmentの語を確定した次のプレーン形式とする。

`Brief対象N件｜重要度「高」N件｜本日確認N件｜今週確認N件`

未判定が1件以上の場合だけ末尾へ`｜未判定N件`を加える。「本日の状態」ラベル、括弧、コロン、文末の句点は付けない。

先頭segmentは`Brief対象`であり、`掲載`ではない(BL-048)。この`N`は`select_brief_eligible_items()`適用後のBrief対象母集団の件数であり、§7.4のdashboardが表示する`掲載 N件`(metadata_only相当を含む全記事card数)とは母集団が異なる。同一ページ上で同じ語が異なる母集団を指さないよう、Brief側では無限定の「掲載」を使わない。件数の算出方法・selection・metadata_onlyのpolicy挙動はBL-048で変更していない(表示語のみの変更)。`Brief対象`はeligibility母集団であってAI成功件数ではない(fallback・未判定を含みうる)ため、「AI分析済み」「AI評価済み」とは書かない。

概況本文(状態行に続く決定論的な説明文)も、全記事cardではなくBrief対象集合の話であることが分かる表現とする。metadata_only cardが存在する日に全掲載記事を評価したと読める表現は使わない。

旧daily JSONの表示互換では保存値を書き換えず、日別Archiveの表示時だけ同じ現行形式へ変換または補完する。表示時の正規化対象は**状態行と概況本文の両方**である。

- **状態行:** 保存済みの`掲載N件｜…`(pre-BL-048)および`本日の状態（掲載N件）：…`(BL-016以前)を、件数を保ったまま現行形式へ正規化する。
- **概況本文:** 認識した状態行が持つ件数(重要度「高」／本日確認／今週確認／未判定)だけを根拠に、BL-048以前の決定論的formatterがその値で生成し得た説明文を厳密に1つ再構成し、概況本文がその文字列で**始まる場合に限り**現行wordingへ正規化する。**状態行の件数・状態と整合しない説明文は正規化しない**(件数不一致、state不一致、および「本日確認が0件でなければ重要度文は付かない」という旧formatterのruleに反する重要度文は、いずれも一致せず不変のまま)。件数はstored textから取り、item dataから再計算しない。一致した場合も、説明文の後ろに続く本文(Gemini生成の自由文等)はbyte-for-byteのまま保持する。

厳密一致しない自由文overviewは従来どおりfail-openで全文表示し、書き換えない。
...
## 19.6 head metadata: document titleとmeta description（Version 1.9、承認済み）
...
可視layoutを変更しないため、Version 1.9は**目視受入（screenshot）を必要としない**。受入証跡は、生成物から取り出した4ページ種別の実`<title>`／description、全公開HTMLのdescription 1件ずつ、および「`<title>`とdescriptionの行を除去すると各HTMLがbaselineと同一」というnormalized comparisonとする。**2026-08-14、この証跡に基づき独立実装レビューがACCEPT（Blocker 0）となり、ユーザーが実装受入を完了したため、Version 1.9を承認済みとした**（accepted implementation head `a8670d3f303c893f0346e54cda9ac3e699aceb0a`、[PR #123](https://github.com/matkei31/security-digest/pull/123)）。仕様そのものの承認は実装着手前に得ており、ここで確定したのは**実装の受入**である。

## 19.7 head metadata: rel="canonical"（Version 1.10、承認済み）

### 19.7.1 なぜ本書の対象なのか

`rel="canonical"`は**全公開ページの`<head>`に置く1要素**であり、19.6がすでに規定しているdocument title・meta descriptionと同じsurfaceに属する。19.6.4は`rel=canonical`を「Phase A-2のscope外」として名指ししており、本書がこの論点を自身の管轄として認識していた。これに対し[BL-009](BACKLOG.md#bl-009--seoと閲覧者増加策) Phase A-3の`robots.txt`／`sitemap.xml`は**ページの`<head>`ではなく独立した公開ファイル**であるため本書へは記録していない。この区別に従い、canonicalは本書のhead metadata contractとして記録する。可視表示は一切変更しない。

### 19.7.2 preferred URL（exact）

全公開HTMLは`<link rel="canonical" href="...">`を**ちょうど1件**持つ。値は**絶対URL**で、Phase A-3のsitemapに載るpreferred URLと**完全に同一の集合**である。

| ページ種別 | canonical |
|---|---|
| トップ | `https://monomidigest.com/` |
| Archive一覧 | `https://monomidigest.com/archive/` |
| About | `https://monomidigest.com/about.html` |
| 日別Archive | `https://monomidigest.com/archive/{YYYY-MM-DD}.html`（各日が自分自身の日付URLを指す） |

- ディレクトリrootをpreferred URLとし、**`/index.html`形式をcanonicalにしない**（公開実測でも`/`と`/index.html`、`/archive/`と`/archive/index.html`はbyte-identicalなaliasである）。
- canonicalにはHTTPS・apex `monomidigest.com`のみを使用する。www・旧GitHub Pages URLはcanonicalに使用しない。wwwは公開実測で301によりapexへ集約されている（旧GitHub Pages URLのredirectはPhase A-4では実測していない――canonicalに使用しないというcontractのみを維持する）。
- **トップページと最新日の日別Archiveは、内容が一時的に類似しても別ページとして扱う。** トップは`/`を、日別Archiveは自身の日付URLをcanonicalとし、互いを指さない。

### 19.7.3 originの正本

公開originはPhase A-3で導入した`fetch.py`の`PUBLIC_ORIGIN`を唯一の正本とし、canonical専用のorigin定数は新設しない。sitemapのURLとcanonicalは`PUBLIC_ORIGIN`と、ページ種別ごとのpreferred path（`TOP_PAGE_PATH`／`ARCHIVE_INDEX_PATH`／`ABOUT_PAGE_PATH`と日付から導く日別path）という同一のsourceから組み立てる。`docs/CNAME`との一致contractは維持し、testが保証する。

### 19.7.4 Phase A-4のscope外

Open Graph、Twitter Card、favicon／`rel=icon`、structured data（JSON-LD）、`meta robots`、RSS/Atom配信は**Phase A-4では追加しない**。これは「今回追加していない」という事実であり、将来追加してはならないという契約ではない（testでもこれらのabsenceを固定しない）。title・meta descriptionのcontract、可視H1・body copy・layout・navigation・analytics beacon／footer、`robots.txt`と`sitemap.xml`のcontractも変更しない。
...
Google公式によれば、Google Searchでfaviconの表示対象になるには**homepageのhead内にfaviconを示すlinkを置く**必要があり、**Googlebot-Imageがfavicon fileを、Googlebotがhomepageをcrawlできる**ことが前提となる。faviconは**search appearance／site attribution**に関わる要素として扱う。またGoogle公式は「すべてのガイドラインを満たしても検索結果への表示は保証されない」と明記しており、本書もそれ以上を主張しない。**本Phaseはranking benefitを目的にも前提にもしない**（本書は「ranking要因ではない」とGoogleが明示した事実としては記載しない）。
...
可視body・layoutを変更しないため、Version 1.11は**ページのscreenshotによる目視受入を必要としない**。favicon自体は視覚要素であるため、受入証跡は**asset（`docs/favicon.svg`）そのものの確認**と、全公開HTMLが同一の`rel="icon"`を1件ずつ持つこと、favicon行を除去すると各HTMLがbaselineと同一であるというnormalized comparisonとする。**2026-08-15、この証跡に基づき独立実装レビューがACCEPT（Blocker 0）となり、ユーザーが実装受入を完了したため、Version 1.11を承認済みとした**（独立レビューがACCEPTしたhead `46ebe21e649c18835956ff10f1b196503e018122`。実際のaccepted implementation headは、これに最新mainのscheduled production同期とレビュー指摘のMinor修正3件・受入記録を重ねたPR最終headである。[PR #129](https://github.com/matkei31/security-digest/pull/129)）。受入対象は[BL-009](BACKLOG.md#bl-009--seoと閲覧者増加策) Phase A-6のみであり、BL-009 umbrella自体は進行中である。
