...
本書は、Monomi Digestの現行UIについて、すでにユーザーが選択・受入した仕様、安定した意思決定、現在の`main`実装と回帰テストを一つにまとめる。将来のUI変更で参照するリポジトリ常駐の正本とし、同じ論点を都度推測し直すこと、受入済みの判断を未決へ戻すこと、Fable 5提案を無条件に再採用することを防ぐ。
