...
本書は、Security Digestの現行UIについて、すでにユーザーが選択・受入した仕様、安定した意思決定、現在の`main`実装と回帰テストを一つにまとめる。将来のUI変更で参照するリポジトリ常駐の正本とし、同じ論点を都度推測し直すこと、受入済みの判断を未決へ戻すこと、Fable 5提案を無条件に再採用することを防ぐ。
...
Draft 0.1全体はユーザーレビュー待ちである。ただし本文の「確定仕様」は、Draft作成前から存在する受入済み判断または現行契約を統合したものであり、Draftで新たに採用したFable 5提案ではない。
...
本書では、上記1〜3で根拠を確認できた項目を「確定仕様」、現行コードにある具体値のうち個別のユーザー承認を確認できないものを「現行値」、採否の証拠がない提案を「未決事項」と表記する。未決事項を確定仕様として扱ってはならず、実装へ進める前にユーザー裁定が必要である。

主要な根拠は[SD-012](DECISIONS.md#sd-012--dashboard-v2-priority-index-and-the-article-reason-no-imperative-contract)、[SD-013](DECISIONS.md#sd-013--ordinary-article-card-variant-b-remove-classification-label-badges-keep-関連タグ-round)、[BL-016](BACKLOG.md#bl-016--本日の状態ラベルを除去する)、[BL-017](BACKLOG.md#bl-017--過去ダイジェストの回遊性と一覧表示を改善する)、[BL-018](BACKLOG.md#bl-018--トップページとjson再構築時の記事時刻表示を一致させる)、`fetch.py`、`test_fetch.py`、`test_archive.py`である。
...
- AI処理であることを隠すことを目的としない。一方、AI利用を明示する注記の具体的な文言と配置は未決事項である。
...
8. 日別Archiveの最下部ナビゲーション。実在する前後日がある場合だけ表示

Brief、優先確認、dashboardはそれぞれ「概況」「短い索引」「全体集計」を担当し、記事カード本文を複製しない。

### 4.2 Archive一覧

`header`に「過去のダイジェスト」、最終更新（値がある場合）、「最新のダイジェストへ戻る」を置き、その後に日付の新しい順でArchiveカードを並べる。
...
- トップページの可視見出しは現行の`🔐 Security Digest`、日別Archiveは`Security Digest`、Archive一覧は`過去のダイジェスト`である。名称変更はBL-006まで行わない。
...
ヘッダーpaddingはPC／モバイルとも`20px 16px 16px`、見出しは`18px`である。600px以下専用のヘッダー圧縮は実装されていない。絵文字削除とモバイルsticky header圧縮は、ユーザー裁定まで未決事項とする。
...
- 現行CSSは`overflow-wrap:anywhere`で折り返し、モバイルで英語原題を行数clampしない。clampの採否と行数は未決事項である。
...
- 現行の`.kev-badge`はアンバー系の小さいpillである。Fable 5が提案したKEV形状の正確な案と、その案に対する明示的なユーザー裁定はリポジトリに残っていないため、将来形状は未決事項とする。
...
「本日の要点あり」「本日の要点なし」と、その代替ラベルは表示しない。日付の新しい順に並べ、同一日付は重複表示しない。0件の場合は「公開済みのダイジェストはありません。」と表示する。「最新のダイジェストへ戻る」は維持する。

### 14.2 日別Archive

- 上部には「最新のダイジェストへ戻る」「過去のダイジェスト一覧へ戻る」を常に置く。
- 上部と最下部の両方に、実在する直前・直後のArchiveへの「前のダイジェスト」「次のダイジェスト」を置く。
- カレンダー上の前日・翌日ではなく、検証を通過したdaily JSONの日付一覧に存在する日だけをリンク対象にする。日付欠損を飛び越える。
- 最古の日は「前」を表示せず、最新の日は「次」を表示しない。
- 前後日が一つもない場合、最下部ナビゲーションは表示しない。
...
- 現行ヘッダーはモバイルでもstickyで同じpaddingを使う。圧縮案は未決事項である。
- 現行の英語原題はモバイルでもclampせず折り返す。clamp案は未決事項である。
...
掲載0件日の各部品の現行挙動はテストで固定されているが、Fable 5が提案した「0件日のページ全体」の別レイアウトを採用するかは未決事項である。
...
- 現行CSSはfocus outlineを消しておらず、ブラウザ既定のfocus表示を使う。将来のCSS変更でfocus表示を除去する場合は、同等以上に見える代替を同時に定義する。
- 現行はlink hoverにunderlineを出す。専用の`:focus-visible`意匠は未決事項である。
...
- [ ] 2026-07-14のような中間日で、上部と最下部の前後リンクが実在Archiveへ移動する。
- [ ] 最古日に「前」、最新日に「次」がない。
...
「採用・実装済み」は、少なくとも現行`main`で実装を確認できる区分である。Briefのアンバー枠とKEV掲載表示について、Fable案そのものを名指しした独立のユーザー採用発言はリポジトリ内で確認できないため、将来の見た目変更まで固定する根拠にはしない。
...
| Fable 5の論点 | 現状 | 未決とする理由 |
|---|---|---|
| AI利用を明示する注記 | 専用注記なし | 文言、配置、適用範囲のユーザー裁定がない |
| モバイルsticky headerの圧縮 | 600px以下も同じpadding | 明示的な採否記録がない |
| ヘッダー絵文字の削除 | トップページは`🔐 Security Digest` | 絵文字単独の採否記録がない |
| モバイルでの英語原題clamp | clampなし、自然折返し | 行数と省略時の到達方法を含む裁定がない |
| KEV表示の形状 | 現行はアンバー系pill | Fable案の正確な目標形状とユーザー裁定が残っていない |
| 掲載0件日の全体表示 | 各セクションの現行空状態を併置 | ページ全体を別構成にする案の採否記録がない |

### 19.2 裁定に必要な選択肢

| 未決事項 | 選択肢 | 現状 | 変更時の主な影響 |
|---|---|---|---|
| AI利用注記 | 追加しない／ページ共通に追加／分析区分ごとに追加 | なし | 文言、責任範囲、全ページの縦量 |
| モバイルsticky header | 現状維持／paddingと情報を圧縮／sticky自体を解除 | 同じpaddingでsticky | anchor offset、表示領域、Archive導線 |
| ヘッダー絵文字 | 維持／削除／別記号へ変更 | トップに`🔐` | ブランド表現、browser title、トップ／Archiveの統一 |
| 英語原題clamp | clampなし／固定行数で省略／開閉手段を付ける | 自然折返し | 長いタイトルの可読性、カード高さ、全文到達性 |
| KEV形状 | 現行pill／プレーンテキスト／別の非pill強調 | アンバー系pill | 客観情報の強調度、関連タグとの形状重複 |
| 掲載0件日の全体表示 | 現行の各空状態／専用の一括空状態 | 各セクションを表示 | 情報階層、Brief／dashboard／収集元の表示有無 |
| focus意匠 | browser既定／共通`:focus-visible`を追加 | browser既定 | キーボード可視性、全リンクの色・outline |

### 19.3 根拠不足の範囲

リポジトリ内にはFable 5レビュー成果物そのものが保存されていない。このため、次はDraft 0.1では分類をこれ以上細分化できない。
...
これらを推測で補わず、裁定時は既存のレビュー資料または新たなユーザー判断を根拠として追記する。

## 20. 変更管理

1. UI変更を行うチケットは、着手時に本書の確定仕様、現行値、未決事項を確認する。
2. 未決事項はユーザー裁定前に実装しない。裁定内容はBACKLOGの原文・受入証跡と、必要に応じてDECISIONSへ記録する。
...
7. Draft 0.1はユーザー承認後に版と状態を更新する。承認前は、Draft自体をBL-004完了の証拠にしない。

### 20.1 版履歴

| 版 | 状態 | 内容 |
|---|---|---|
| Draft 0.1 | ユーザーレビュー待ち | dashboard v2、記事カードvariant B、BL-016〜018、現行実装・テスト、Fable 5提案の採否／未決を初めて統合 |
