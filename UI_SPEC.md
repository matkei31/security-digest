...
- **概況本文:** 状態行の直後が、BL-048以前の決定論的な説明文(A complete／A incomplete／B complete／B incomplete／C complete／C incomplete の6形式と、任意付加される重要度文)に**厳密一致するprefix**である場合に限り、現行wordingへ正規化する。件数はstored textから取り、item dataから再計算しない。説明文の後ろに続く本文(Gemini生成の自由文等)はbyte-for-byteのまま保持する。
