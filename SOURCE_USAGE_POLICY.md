...
- 本Ticket(BL-031)は監査・方針文書の整備のみを対象とする。ここで提案するcontent usage modeをproductionコードで強制する実装(`source_definitions.json`への`content_usage_mode`等のfield追加、`fetch.py`側の共通処理)は、後続のBL-032で行った(BL-032は[PR #69](https://github.com/matkei31/security-digest/pull/69)としてmerge済み。10章参照)。
...
**BL-032実装状況(実装受入済み、[PR #69](https://github.com/matkei31/security-digest/pull/69) merge済み・`main`で有効。10章参照):**
...
**表示(BL-032実装状況、実装受入済み、[PR #69](https://github.com/matkei31/security-digest/pull/69) merge済み・`main`で有効。10章参照):**
...
## 7. Output-similarity and quotation controls (BL-032必須要件・実装済み、PR #69 merge済み。10章参照)

このcontrolは、決定論的な仕組みで機械的に強制できる要件と、追加のモデルなしでは完全な自動検出を約束できない意味的な残余リスクとを区別して記録する。両者を同列に「すべて自動検出する」と扱わない。

### A. 機械的に強制可能なBL-032要件

次は、決定論的な処理(文字数制限、文字列一致検出、必須フィールド検証)で機械的に強制できる要件として記録する。BL-032の実装([PR #69](https://github.com/matkei31/security-digest/pull/69) merge済み)により、以下は`daily_json.py`／`fetch.py`で実装済みである(具体的な閾値・実装箇所は[BACKLOG.md](BACKLOG.md#bl-032--取得元別content-usage-policy-enforcement)・[`test_content_usage_policy.py`](test_content_usage_policy.py)参照)。
...
具体的な閾値(文字数・verbatim long match最小長等)は、BL-032の実装([PR #69](https://github.com/matkei31/security-digest/pull/69) merge済み)で決定・テストと共に一元管理済みである(意味的類似度スコアのような追加モデルは導入していない)。本文書(BL-031)自体は監査・方針の記録にとどまる。
...
- **反映:** `google_tag`・`mandiant`の`checked_at`を2026-07-30へ更新し、confidence(medium)・分類(`feed_summary`, conditional)は変更していない。BL-032の実装([PR #69](https://github.com/matkei31/security-digest/pull/69) merge済み)は、公式Feedのみの利用・記事ページ追加取得の禁止・rich content不使用をコードで機械的に強制するが、machine-readable instructionsの変更をrecheck triggerとして自動検知する仕組みは実装しておらず、引き続き`SECURITY_OPERATIONS.md`の人による運用手順に依る。
...
- **[BL-032](BACKLOG.md#bl-032--取得元別content-usage-policy-enforcement)(完了、独立レビュー7ラウンド・合計12件のBlockerすべて修正・独立再確認済み、[PR #69](https://github.com/matkei31/security-digest/pull/69) merge済み〔merge commit `cd5e6ec5d08542d6eb76a134944d1729e5d5f4dd`〕)**: 本文書で提案したcontent usage modeを、`source_definitions.json`への`content_usage_mode`等の設定項目追加と、`fetch.py`側の共通処理(取得・Gemini入力・保存・公開の各段階でのmode強制)として実装し、`main`で有効化した。Gemini data-use gate(5章)の`paid_verified`確認結果を反映済み。output-similarity/quotation controls(7章)を実装しテスト済み(`test_content_usage_policy.py`)。本文書の監査結果・方針自体(Version 0.1 Approved)は本mergeによって変更されない。このmerge自体は、次回scheduled production run・`workflow_dispatch`・実Gemini API呼び出しをこのタイミングで実行することへの事前承認ではなく、次回scheduled production runは通常運用のスケジュールに従い、この新しいenforcement契約(schema_version=2を含む)のもとで生成される。
...
- 4章の監査表にある`proposed_mode`列は、Approved監査時点の分類提案を示す歴史的な列名として維持する(BL-032の受入・merge後も列名自体は変更しない)。BL-032の実装([PR #69](https://github.com/matkei31/security-digest/pull/69) merge済み、merge commit `cd5e6ec5d08542d6eb76a134944d1729e5d5f4dd`)は、この提案値をそのまま`source_definitions.json`の`content_usage_mode`へ反映し`fetch.py`で強制しており、この反映・強制は`main`上で有効である。
