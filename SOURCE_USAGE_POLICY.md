...
`fetch.py`の`generate_archive_outputs()`は、Archive生成の直前に`daily_json.validate_daily_digest_for_archive_read()`(Archive読込専用のschema-version-awareなvalidator)を実行する。schema v2は保存前と完全に同じstrict validation(`validate_daily_digest()`、NCSC attribution snapshot検証を含む)をそのまま適用するが、schema v1(レガシー)は、現行の閾値・enum・field構成(例: 現行の`BRIEF_MAX_CHECK_ITEMS`)を実在ファイルへ遡及適用せず、安全にHTMLへ描画するための最低限の構造だけを検証する――これは、生成当時は正当だった実データ(例: 現行upper boundより多い件数のBrief check_itemsを保存している実在のschema v1ファイル)が、後日の閾値変更を理由に誤ってArchive生成対象から除外されないようにするためである。この検証を通過しないdaily JSON(schema v2でNCSC attribution snapshotが欠落・不正な場合等)は、Archive生成(日別Archive HTML・Archive一覧・index entryのいずれも)の対象から除外する。
