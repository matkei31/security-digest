...
`daily_json.validate_daily_digest()`は、schema v2でncsc・structured_open・ai_eligible=trueの記事について、このsnapshotが`is_safe_attribution_url()`を通過することを保存前に必須検証する。この検証を通過しないdaily JSONは、`fetch.py`の`generate_archive_outputs()`がArchive生成(日別Archive HTML・Archive一覧・index entryのいずれも)の対象から完全に除外する(`load_daily_digest()`の緩い検証だけでなく、Archive生成直前に`validate_daily_digest()`によるfull validationを実行する)。これは、`digest_items_for_html()`側のitem単位のdowngrade(記事カード・Dashboard集計・優先確認等)だけでは、保存済みBrief(overview/discussion_points/check_items、`brief_for_html_from_digest()`経由)には及ばないためであり、保存済みBriefも含めた完全な保護は、このvalidationによる生成対象からの除外そのものによって担保する。`digest_items_for_html()`自身が行うitem単位のdowngrade(`downgrade_reason: archive_attribution_snapshot_invalid`)は、この保存前validationを経由しない直接呼び出し(テスト等)に対する二次的な防御的backstopであり、単独では保存済みBriefまでは保護しない。schema v1の既存daily JSON・Archiveはこの仕組みの対象外であり、遡及変更しない。
