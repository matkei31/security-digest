...
- **残作業:** 上記完了条件1〜17は本PRで実装済み(具体的なthreshold値も本PRで決定済み)。独立レビュー(round 1)・独立再レビュー(round 2)・独立レビュー(round 3)・独立レビュー(round 4)・独立レビュー(round 5)で指摘された合計10件のBlockerは、いずれも本PRで修正済み。残る作業はユーザー受入・独立レビューのみ。BL-009の包括的About／訂正窓口UI整備は引き続きscope外・別Ticket。
