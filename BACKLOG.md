...
- **実装証跡:** [PR #9](https://github.com/matkei31/security-digest/pull/9), merge commit `82b23c720b5871c5f46d068813defc12af164e4a`；現行実装（`fetch.py`内の`split_brief_overview_status_line()`と「本日の要点」見出し）；`test_fetch.py`/`test_archive.py`の回帰テスト。BL-016残対応として、決定論的な状態行を「本日の状態」ラベル・括弧・コロン・文末の句点を除いた｜区切り形式（`掲載{件数}件｜重要度「高」{件数}件｜本日確認{件数}件｜今週確認{件数}件`、未判定segmentは既存条件のまま0件超の場合のみ付加）へ変更した（`format_brief_status_line()`）。既存data/JSONに保存済みの旧形式overviewは書き換えず、表示時にのみ数値を保ったまま現行形式へ変換する後方互換ロジックを`split_brief_overview_status_line()`へ追加し、既存の全日別archive HTML（`docs/archive/2026-07-11.html`〜`2026-07-18.html`および`docs/archive/index.html`）をGemini・外部HTTP・記事再分析を行わずに既存daily JSONと現行テンプレートから再生成した。決定論的状態行を含む2026-07-15〜2026-07-18の4ファイルのみ表示内容が変わり（状態行の表記のみ）、記事内容・件数・順序・分析結果および`data/`配下のJSON内容は変更していない（2026-07-11〜07-14はTicket 15b以前の自由文形式のoverview、または07-13はbrief自体が存在しないため、既存のfail-open表示のまま変更なし）。
- **ユーザー受入証跡:** PC/390px実装の見た目について、ユーザーはverbatimで次のとおり述べた：「見た目いいと思うよ。「本日の状態」という言葉があんまりかっこよくないから、いくつか案出して検討したいね。」。ラベル案の検討を経て、ユーザーは次のとおり明示的に文言決定した：「うん。「本日の状態」は「1. ラベルをなくす — 最有力」でいこう」。この2件のverbatim発言により、PC/390px表示自体の受入と、「本日の状態」ラベルを除去する文言変更方針の受入は得られている。ラベル除去後の新表示（本番）そのものに対する明示的なユーザー受入は、本項目のcompletion時点ではまだ記録されていない。
- **残作業:** 本番表示（PC／390px）の最終確認と、状態の更新のみ。
- **注記:** 本項目は新規UI実装を提案するものではなく、[BL-014](#bl-014--過去ユーザーコメントの体系的棚卸し) Batch 2で確認された既存の受入ギャップを記録するものである。監査の詳細は[BACKLOG_AUDIT.md](BACKLOG_AUDIT.md) Batch 2を参照。ラベル除去案は、ユーザーが複数案から明示的に選択した文言決定であり、上記ユーザー受入証跡のverbatim発言を出典とする。

## BL-017 — 過去ダイジェスト間を前後移動できるようにする

- **ID:** BL-017
- **タイトル:** 過去ダイジェスト間を前後移動できるようにする
- **優先度:** P1
- **状態:** 仕様化済み / 未実装
- **出所種別:** ユーザー原文
- **ユーザー原文:** 「あと、過去のダイジェストについて、ワンクリックで前日分とかに行き来できる改修はこれから？」
- **ユーザー確認済み要約:** 該当なし — 原文は上記のとおり回収済み。
- **解釈:** 日別archiveのカレンダー上の前日・翌日ではなく、実在するarchive日付（daily JSONが存在し、archiveが生成されている日）の前後へ、ワンクリックで移動できるナビゲーションを追加する。
- **完了条件:** 各日別archiveページのページ上部と最下部の両方にナビゲーションを配置する；移動先は実在するarchive日付のみとし、カレンダー上の前日・翌日ではない；最古のarchiveでは「前」方向のリンクを表示せず、最新のarchiveでは「次」方向のリンクを表示しない；実装コードとテストで検証し、ユーザー受入を得る。
- **依存関係:** 既存の`docs/archive/*.html`生成ロジック（`build_daily_archive_html()`、`generate_archive_outputs()`）；`data/index.json`のarchive日付一覧。
- **実装証跡:** 未実装。本PR（BL-016残対応）ではBL-017を実装しない。
- **ユーザー受入証跡:** 記録なし。
- **残作業:** ナビゲーション配置の設計、実在日付の前後移動ロジックの実装、最古・最新での境界表示制御、テスト、ユーザー受入。
- **注記:** 本項目は今回のBL-016残対応PRの範囲外であり、別途実装する。
