...
- **tranche 3c最終受入:**
  ...
  - **`git diff --check`:** success
  - **changed files:** 5件
  - **diff:** 868 insertions／172 deletions
  - **unresolved review threads:** 0
...
- **実装証跡(tranche 3d):** tranche 3b/3c で確立した基準(document context・meaning-preserving reflow/rewordに対する脆弱性で分類する、raw negative multi-token substringはCとする、structural regexは reformatting-immuneなためBとする、`- **Status:**`等のenum-like固定fieldはB、`- **Date:**`等のexact identifying valueはD、fingerprint重複のない単発のexact machine-readable valueはA化せずDとする)を全98件へ適用した。判定にあたり、STATUS.mdの「Current versions」節・「Sources of truth」節がdata/index.json等への正本委譲を*narrative prose*で説明する文書であることを確認し、`正本とする`・`Git履歴`・`source of truth`等の記述的phraseは、table row内であっても(Markdown tableのcellは改行できないため厳密な意味でのreflow対象ではないが)reword可能な自由記述であるため、tranche 3c round 2で確立した「短いという理由だけでB/Dにしない」原則に従いCとした。一方、`data/index.json`等のpath参照・`- **Status:** Accepted`等のenum-like固定field・structural regex checkはBのまま維持し、exact SHA／PR／CI run／Pages deployment run／日付／schema versionはDとした。最終分類: **test_status.py: A 0／B 31／C 39／D 28(total 98)**。`document_test_classification.json`を`scope: [test_custom_domain.py, test_ui_spec.py, test_status.py]`・380 entries(既存282 entriesはbyte-identicalに無変更、新規98 entriesをsource順で追記)・390行へ拡張し、`document_test_inventory.py --check`成功(combined A=18 B=127 C=164 D=71、per-file test_custom_domain.py=97/test_ui_spec.py=185/test_status.py=98、unclassified/stale/fingerprint mismatch いずれも0)を確認した。`test_document_test_classification.py`を26 testsのまま(新規追加なし、既存testを3-file scopeへ一般化)拡張し、`CUSTOM_DOMAIN_EXPECTED_A/C/D_IDS`・`UI_SPEC_EXPECTED_A/C/D_IDS`・既存282 entriesの期待値は無変更のまま、`STATUS_EXPECTED_A_IDS`(空集合)/`STATUS_EXPECTED_C_IDS`/`STATUS_EXPECTED_D_IDS`を新規追加し、combined exact membership guardを3-file unionへ拡張した。scope shrinkage mutation testへ`class-shrink-within-status`・`file-shrink-drop-status-entirely`の2シナリオを追加し、file order swap mutation testを3-fileの全swap pair(0,1)(1,2)(0,2)へtable-driven化し、assertion order swap mutation testを3-file全ファイルでtable-driven化して`test_status.py`内assertion順序swap検出を追加した。既存の削除・追加・fingerprint改変・category-action不整合の4種のmutation testは`SCOPE_FILES`タプル経由で自動的に3-fileへ対象拡大した。mutation-style verification(commitしない一時変更、対象document `STATUS.md`・`DECISIONS.md`のみ、検証後にすべて復元)により、Category C該当の代表例(`正本とする`のreword)が実際に`assertIn`を壊すこと、Category B該当のstructural regex(`Version\s+\d+\.\d+`)がharmless contextでは安全なまま、load-bearingな`Version 1.2`の再導入では正しく検出されること、Category D該当のSD-031 exact date(`2026-08-01`)が日付変更で正しく検出されることを確認した。全mutationは検証後に復元し、working tree差分が意図したファイルだけであることを確認した。Category C該当39件はこのPRでのsource変換対象ではない(action `refactor_later`をmanifestへ記録するのみ、変換は別途ユーザー承認後の残作業)。
- **残作業:** Category C conversion(tranche 3bの41件・tranche 3cの84件・tranche 3dの39件、方針・実施は別途ユーザー承認後)、CJK prose normalizationのnuance(space-collapsingではなくspace-strippingが必要、round 1レビューで確認)、tranche 3dのDraft PR独立レビュー・最終受入・Ready化・merge、`test_content_usage_policy.py`(runtime／behavioral scope、後続で別途判断)、`test_source_definitions.py`、約1593件のfull per-assertion classification残り、multiline／120文字以上literal heuristic以外のbrittleness確認、Category A(tranche 3b 8件・tranche 3c 10件)のhelper consolidation要否判断、残candidateのmutation-style verification、BL-038全体の最終受入。tranche 1・2・3a・3b・3cは受入済み、tranche 3dは実装中(Draft PR作成・独立レビュー・最終受入は未実施)であり、BL-038全体の最終受入は上記残作業が完了するまで行わない。
