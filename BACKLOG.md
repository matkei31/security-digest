...
- **状態:** 実装中(tranche 1・2・3a受入済み／tranche 3b実装中)
...
- **ユーザー原文の履歴(「ok」4回・「おk」3回が別発言として登場、出現回数ではなくnumbered entryのrole・orderで区別し、混同・上書きしない):**
...
- **tranche 3b着手(2026-08-05):** tranche 3b kickoff原文「ok」(着手時原文「ok」・tranche 2 kickoff原文「ok」・tranche 3 kickoff原文「ok」のいずれとも同一文字列だが別の時点・別の対象への別発言であり、混同・上書きしない)。branch `test/bl038-tranche3b-custom-domain-classification`(origin/main `bfa6c7281c760597b865a425de2f3df6759c1a3d`、[PR #82](https://github.com/matkei31/security-digest/pull/82) merge commitから新規作成)。scope: `test_custom_domain.py`のみ。tranche 3開始時に作成したlocal-only WIP commit(`8435dcc32518037b96b736ad7f81e4a1b951c348`、未push)の`document_test_classification.json`を advisory参考値としてのみ抽出し利用した(A 12／B 81／C 0／D 4、97 assertion、compact manifest約105行)。tranche 3aでcustom helper transitive fingerprint・schema exactness等が大幅に強化されたため、最新main上のsource・infrastructureから全97 assertionを再列挙・再fingerprint化した(WIP commit自体はcherry-pickせず、旧manifestもそのままcommitしていない)。最新inventory実測: scoped classes 7件(`DocsCnameFileTest`・`CnameSurvivesGenerationTest`・`ArticleBriefContractUnchangedTest`・`Bl007DocumentationTest`・`ReadmePublicUrlTest`・`Bl007ClosureRecordTest`・`TicketIdTypoTest`、source order)、distinct test method 40件、assertion総数97件(advisory値と一致、infrastructure変更によるcount変化なし)。97件すべてをsource context付きで個別レビューし、旧prototype値と比較したところ21件で分類が変わった: 旧prototypeのA12件中9件をB(4件: state/status field等の非重複短文)またはD(5件: 実存する2件のcommit SHA・branch名・CNAME byte-exact contentの計3件+version identifier 3件の重複誤判定分)へ訂正し、新たに9件をA(fingerprint完全一致による真のcross-method/cross-class重複3組: CNAME `is_file()`存在確認3箇所、wildcard DNS関連文言2組、意図しない追加commitなし文言1組)へ追加した(net A総数は12件で偶然一致するが構成は総入れ替え)。旧B81件中さらに2件(SD-028のevidence key:value literal `protected_domain_state: verified`／`https_enforced: true`)をDへ訂正した。訂正後の最終分類: **A 12／B 74／C 0／D 11(total 97)**。C 0は妥当と判断した(このfileにmultiline／line-wrap依存のbrittle literalは存在せず、97件すべてが単一行のsubstring/exact/ordering契約である)。分類方針は現在mainの`document_test_inventory.py`のcategory/action mapping(A→`keep`は誤り、正しくはA→`keep`,B→`already_structural`,C→`refactor_later`,D→`historical_keep`。実装済みmapping通りに記録)と`test_document_test_inventory.py`のschema contractを正本として使用した。Category C該当は0件のためこのPRでのsource変換は発生しない。97 entry全件へfile/class/method/source-context個別のcontract_summaryとrationaleを記載し(rationale 97件すべて相互に異なる文字列)、generic同一文での穴埋めはない。
  - **manifest:** `document_test_classification.json`(新規、valid JSON、schema_version 1、pretty top-level+scope、compact 1-entry-1-line assertions、105行、target style統一(全97件`target`単数形、`targets`不使用)、`document_test_inventory.py --manifest document_test_classification.json --check`実行結果success(scoped files 1・classes 7・assertions 97・A=12 B=74 C=0 D=11・unclassified/stale/fingerprint mismatch いずれも0))。
  - **structural scope/count guard:** 新規`test_document_test_classification.py`(16 tests)で、declared scope(file `test_custom_domain.py`・7 classをhardcoded literal tupleとして固定)・assertion数97(manifestとlive inventory双方を独立に固定値97と比較)・manifest IDと live inventory IDのexact一致・file/class/method/ordinal/assertion_api/fingerprintの一致・`validate_manifest()`failures空・corrected category counts(A12/B74/C0/D11)のexact固定・action-category mapping・target style統一・summary/rationale非空かつ97件相互に異なることを検証した。scope縮小(class 1件をscopeとassertions両方から除去した自己無矛盾なmanifest)はvalidator単独では検出できないことを確認したうえで、hardcoded expected-class literal tupleとの比較によりこの縮小を検出できることをmutation testで実証した(item 20)。加えてentry削除→`unclassified`、余分entry→`stale-entry`、fingerprint改変→`fingerprint-mismatch`、category/action不整合→`category-action-mismatch`をそれぞれmutation testで確認した(items 21-24)。
  - **mutation-style verification(repositoryへcommitしない一時的な変更、実施後は必ず復元):** harmless source mutation(assertTrueの括弧位置変更)ではfingerprint不変・manifest check success、semantic source mutation(`assertTrue`→`assertFalse`)では`fingerprint-mismatch`/`assertion-api-mismatch`で検出、inventory drift(assertion追加は`unclassified`、削除は`stale-entry`、method rename は`unknown-method`)、manifest drift(entry削除は`unclassified`、fingerprint改変は`fingerprint-mismatch`、category/action不整合は`category-action-mismatch`)をすべて確認した。全mutationは検証後に復元し、`git status --short`が意図した新規2 file(`document_test_classification.json`・`test_document_test_classification.py`)のみであることを確認した。
  - **source変換:** このPRでは`test_custom_domain.py`を一切変換していない(Category C該当が0件のため変換対象自体が無い)。
  - **tranche 3b final acceptance:** 未実施(pending)。
- **残作業:** tranche 3b final acceptance、tranche 3c(`test_ui_spec.py` 185 assertionのclassification manifestとrecord test、未着手、別branch／別Draft PR予定)、`test_content_usage_policy.py`(runtime／behavioral scope、後続で別途判断)、`test_source_definitions.py`、約1593件のfull per-assertion classification残り、multiline／120文字以上literal heuristic以外のbrittleness確認、BL-038全体の最終受入。tranche 1・2・3aは受入済み、tranche 3bはinfrastructure実装(manifest・structural test)完了だがfinal acceptance未実施であり、BL-038全体の最終受入は上記残作業が完了するまで行わない。
