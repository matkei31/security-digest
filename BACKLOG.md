...
- **実装証跡:** 新規`SOURCE_USAGE_POLICY.md`(Version 0.1、Status Draft)を作成し、目的・法的位置づけ・4 content usage mode定義・17 source監査表(structured_open 5／feed_summary 4／metadata_only 4／disabled_legal_review 4、全17でallow_rich_content=false)・Gemini data-use gate(`gemini_data_use_status: unknown`)・attribution要件・output-similarity/quotation control要件(BL-032実装事項)・recheck trigger・unknowns・BL-032/BL-009関係を記録した。`source_definitions.json`のDark Reading(`dark_reading`)を`enabled: false`・`planned_phase: "保留"`へ変更し、Informa TechTarget Terms(公式URL・確認日2026-07-29・具体的な禁止事項・「法的違反を確定したものではなく」の明記)を根拠とする`activation_condition`・`notes`を追加した。他16 source(CrowdStrike・Cloudflareを含む)の設定は変更していない。結果、総数17・enabled 12・disabled 5(`cisa`、`crowdstrike`、`cloudflare`、`dark_reading`、`nist_nvd`)。`SECURITY_REQUIREMENTS.md`をVersion 1.5(Status Draft)へ更新し、非公式翻訳経路・`docs/translate_cache.json`の現行アーキテクチャ記載を削除し、稼働中のカスタムドメイン(`monomidigest.com`)記録・`SOURCE_USAGE_POLICY.md`参照を追加し、新規SR-044〜SR-046・GAP-016・GAP-017を追加した(既存SR/GAP IDは変更・欠番なし)。GAP-011の記述も稼働中の状態に合わせて更新した。`SECURITY_OPERATIONS.md`をVersion 1.1(Status Draft)へ更新し、「Translation cache」節を「Source suspension」節へ置き換え、`docs/translate_cache.json`関連の記載を削除した。`BACKLOG.md`(本エントリ)・`STATUS.md`のActive workへBL-031を追加した。`DECISIONS.md`は変更していない(SD-030は追加せず)。新規`test_source_usage_policy.py`(14 tests)、`test_source_definitions.py`への`Bl031SourceTermsAuditTest`(8 tests)、`test_security_requirements.py`への`Bl031SecurityRequirementsReconciliationTest`(7 tests)、`test_security_operations.py`への`Bl031SecurityOperationsReconciliationTest`(4 tests)を追加し、既存test(`test_archive.py`のfooter件数13→12、`test_fetch.py`のBL ID範囲、`test_security_requirements.py`／`test_security_operations.py`の既存GAP-012・Version 1.4/1.0関連の既存assertion)を現状へ整合させた。`fetch.py`／`daily_json.py`／`vulnerability_facts.py`／`data/*.json`／`docs/index.html`／`docs/archive/*.html`／`.github/workflows/`は変更していない。production・`workflow_dispatch`・Gemini API・外部アクセスは行っていない。**追記(Gemini owner verification反映):** 2026-07-29、repository ownerがGoogle AI StudioのAPI Keys画面で、Monomi Digest productionで使用するAPIキーが属する`security-digest` Projectにactive billingが関連付けられ「Tier 1・前払い」であることを確認した。この結果を受け、`SOURCE_USAGE_POLICY.md`の`gemini_data_use_status`を`unknown`から`paid_verified`へ更新し、`SECURITY_REQUIREMENTS.md`のSR-045を`Met`(旧: `Unverified outside repository`)、GAP-017のdispositionを`Completed owner verification`(旧: `Deferred until trigger`)へ、control mappingの件数を`Met 1 / Partial 2 / Not met 0 / Unverified 0`(旧: `Met 0 / Partial 2 / Not met 0 / Unverified 1`)へ、Section 13のGemini行を`Verified — paid_verified`(旧: `Unverified — owner access required`)へ更新した。`SECURITY_OPERATIONS.md`のVersion 1.1適用済み決定事項へ、この確認が完了した旨と将来のbilling解除／Project変更／APIキー移行時の再確認要件を追記した。記録した非機密情報は「`security-digest` Project」「active billing確認」「Tier 1・前払い」「owner確認日2026-07-29」のみであり、APIキー名・APIキー末尾・APIキー値・Project ID・請求先アカウントID・課金額・画面のスクリーンショットは一切保存していない。Google TAG／Mandiantの取得元規約分類・confidence、および2026-07-30発効のGoogle Terms再確認pendingは変更していない。`source_definitions.json`／`fetch.py`／`daily_json.py`／`vulnerability_facts.py`／`data/*.json`／`docs/*.html`／workflow／`DECISIONS.md`は変更していない。production・`workflow_dispatch`・Gemini API・外部URLアクセスは行っていない。
- **ユーザー受入証跡:** （未受入。本エントリはDraft PR作成時点の記録であり、ユーザー受入は別途行われる。）
- **残作業:** BL-032による`content_usage_mode`実装とfetch.py enforcement、BL-009によるAbout/出典/訂正窓口の実装、Cisco Talos規約適用範囲の確認、Krebs on Securityの公式terms確認(現状は`limited_feed_analysis`としてリスク受容運用)。
- **注記:** 本Ticketは監査・ポリシー文書の作成と既存2文書の整合化に限定される。取得元別のcontent usage mode enforcementをproduction code（`source_definitions.json`のfield追加、`fetch.py`の共通処理）へ反映することは、本Ticketでは実施せず、BL-032として明示的に委譲する。**追記(Google Terms再確認完了・5 mode再編):** 2026-07-30発効のGoogle利用規約について、Google公式のTerms of Serviceアーカイブページで2026-07-30版が最新版として掲載されていることを確認し、repository ownerの通常ブラウザでも日本向け現行規約ページの表示が2026年版へ切り替わったことを確認した(一部取得環境で旧2024年版表示が一時的に残ったことは事実として記録するが、規約発効自体の確認を妨げるものではない)。新規約は引き続きmachine-readable instructionsに反する自動収集を禁止する条件を含み、一般規約のみでAI公開要約が包括的に許諾されたとは断定しない。この確認にAPIキー・billing情報等は関係しない。`google_tag`・`mandiant`の`checked_at`を2026-07-30へ更新し、分類(`feed_summary`, conditional)・confidence(medium)は変更していない。あわせて、content usage modeを4種類から5種類へ再編し、新設した`limited_feed_analysis`(2 source: `the_hacker_news`、`krebs_on_security`)へ再分類した。両sourceは公式RSSの提供は確認できるが包括的な再利用許諾までは確認できておらず、`metadata_only`へ一律に格下げすると実用性が大きく損なわれるため、記事ページ取得禁止・rich content不使用・原文保存禁止・近接翻訳や長い引用の禁止等、高リスク処理を除去したうえで限定運用する。これは「利用許諾を確認した」という判断ではなく、**明示的な運用上のリスク受容**であり、ユーザーが「配慮しつつ使い勝手を可能な限り維持する」方針を選択したことを反映する。`microsoft_security`・`cisco_talos`は`metadata_only`のまま維持する(`metadata_only`は「利用禁止」ではなく、productionの自動処理を原題・取得元・公開日時・原記事リンクに限定する区分であり、人による閲覧・独自報道を妨げるものではない)。件数は structured_open 5／feed_summary 4／limited_feed_analysis 2／metadata_only 2／disabled_legal_review 4、計17。production enforcementは引き続きBL-032。BL-031はまだCompletedではなく、監査・文書Draft PR／レビュー待ちのままである。
