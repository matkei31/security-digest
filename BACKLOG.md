...
- **round 1修正(tranche 3c):** (1)〜(5)・(8)の計6件(D→C)と、(5)は実際には`assert-06`がBだったための訂正1件(B→C)を合わせ、計7件をCategory C／`refactor_later`へ訂正した。各entryのdurable component(exact quote／exact label／numeric values／property set／viewport set)と、rewordable/reflowable wrapper componentをmutation-style verification(commitしない一時変更、対象document `UI_SPEC.md`のみ、検証後にすべて復元)で個別に確認し、raw literalは meaning-preserving edit で壊れる一方、durable componentは維持されることを実証した(全7件で対象文字列の全occurrenceを変更し、未変更の重複箇所による偽陽性を排除)。訂正後のtest_ui_spec.py分類: **A 10／B 61／C 80／D 34(total 185)**(round 1 transition: B→C 1件、D→C 6件)。combined分類: **A 18／B 98／C 121／D 45(total 282)**。`document_test_inventory.py --check`成功(combined A=18 B=98 C=121 D=45、per-file test_custom_domain.py=97/test_ui_spec.py=185、unclassified/stale/fingerprint mismatch いずれも0)。既存97件のtest_custom_domain.py entriesはbyte-identicalなまま無変更。manifest line countは訂正後も291行(rationale文言のみ変更、行数は不変)。(6)は`assertNotIn("未着手", ...)`という汎用禁止を、当該tranche 3cのstale wording(`tranche 3c(...)、未着手`という特定phrase)だけを禁止する狭いassertionへ置き換えた。(7)は`test_manifest_line_count_matches_current_evidence`を新設し、`<=400`のbudget guardとは別に現在の実測line count(291)をexact equalityで固定した。`test_document_test_classification.py`の`UI_SPEC_EXPECTED_C_IDS`/`UI_SPEC_EXPECTED_D_IDS`を訂正後の7件の移動を反映して更新し(`UI_SPEC_EXPECTED_A_IDS`は無変更)、26 testsすべてが成功することを確認した。Category C該当80件はこのPRでのsource変換対象ではない(action `refactor_later`をmanifestへ記録するのみ、変換は別途ユーザー承認後の残作業)。
- **残作業:** Category C conversion(tranche 3bの41件・tranche 3cの80件、方針・実施は別途ユーザー承認後)、CJK prose normalizationのnuance(space-collapsingではなくspace-strippingが必要、round 1レビューで確認)、tranche 3cのDraft PR独立レビュー継続・最終受入・Ready化・merge、`test_content_usage_policy.py`(runtime／behavioral scope、後続で別途判断)、`test_source_definitions.py`、約1593件のfull per-assertion classification残り、multiline／120文字以上literal heuristic以外のbrittleness確認、Category A(tranche 3b 8件・tranche 3c 10件)のhelper consolidation要否判断、残candidateのmutation-style verification、BL-038全体の最終受入。tranche 1・2・3a・3bは受入済み、tranche 3cは実装中(round 1修正まで完了、追加の独立レビュー・最終受入は未実施)であり、BL-038全体の最終受入は上記残作業が完了するまで行わない。
