...
- **残作業:** 上記完了条件1〜17は本PRで実装済み(具体的なthreshold値も本PRで決定済み)。独立レビューで指摘された2件のBlocker(publisher由来description transient処理の不備、structured_open attributionの実表示化不備)は本PRで修正済み。残る作業はユーザー受入・独立レビューのみ。BL-009の包括的About／訂正窓口UI整備は引き続きscope外・別Ticket。
