...
- **状態:** 実装中(tranche 1・2・3a・3b受入済み／tranche 3c以降継続)
...
- **ユーザー原文の履歴(「ok」4回・「おk」4回が別発言として登場、出現回数ではなくnumbered entryのrole・orderで区別し、混同・上書きしない):**
...
- **残作業:** Category C 41件のsource conversion(方針・実施は別途ユーザー承認後)、CJK prose normalizationのnuance(space-collapsingではなくspace-strippingが必要、round 1レビューで確認)、tranche 3c(`test_ui_spec.py` 185 assertionのclassification manifestとrecord test、未着手、別branch／別Draft PR予定)、`test_content_usage_policy.py`(runtime／behavioral scope、後続で別途判断)、`test_source_definitions.py`、約1593件のfull per-assertion classification残り、multiline／120文字以上literal heuristic以外のbrittleness確認、Category A 8件のhelper consolidation要否判断、残candidateのmutation-style verification、BL-038全体の最終受入。tranche 1・2・3a・3bは受入済みであり、BL-038全体の最終受入は上記残作業が完了するまで行わない。
