...
- **状態:** 実装中(tranche 1・2受入済み／tranche 3a実装中)
...
- **ユーザー原文の履歴(同じ日付に「ok」3回・「おk」2回が別発言として登場、出現回数ではなくnumbered entryのrole・orderで区別し、混同・上書きしない):**
...
- **tranche 3a final acceptance:** 未実施(pending)。tranche 3aはDraft PRの作成とCI success確認までを行い、Ready化・merge・tranche 3a final acceptance記録は本Ticketの以降の作業とする。tranche 3b／tranche 3c(pilot classification)は本PRでは着手しない。
- **残作業:** tranche 3以降。約1593件のbroad inventory全件に対するfull per-assertion A/B/C/D classification(未実施)、multiline／120文字以上literal heuristic以外のbrittleness確認(未実施)、必要性が確認された場合の`parse_markdown_table`／`find_ticket_section`／`extract_markdown_subsection`等の追加実装、残candidateのmutation-style verification、BL-038全体の最終受入。**tranche 3b(未着手、別branch／別Draft PR予定):** `test_custom_domain.py`(97 assertion)のclassification manifestとrecord test。**tranche 3c(未着手、別branch／別Draft PR予定):** `test_ui_spec.py`(185 assertion)のclassification manifestとrecord test。**tranche 3以降で扱う残file:** `test_content_usage_policy.py`(1594行・24 class・275 assertion、うち文書／static-contract候補は保守的に見ても41件、Markdown文書を一切読まないruntime-behavioral testが大部分を占めるため今回のpilotから除外、runtime／behavioral scopeとして後続で別途判断する)、`test_source_definitions.py`。tranche 1・2は既知の全candidateを分類・対応済み、tranche 3aはinfrastructureのみ完了だが、BL-038全体の最終受入は上記残作業が完了するまで行わない。
