...
- **実装証跡:** （なし。本エントリは要件定義・登録のみを記録する。実装はこのPR自体では着手していない。）
- **ユーザー受入証跡:** （未受入。本Ticketはまだ実装されていない。）
- **残作業:** 上記完了条件1〜17すべて。
- **注記:** 本PR(branch `docs/bl031-acceptance-bl032-registration`)は`BACKLOG.md`へのTicket登録のみを行うdocumentation-only PRであり、`fetch.py`・`daily_json.py`・`source_definitions.json`・HTML生成・schema・prompt・workflowファイルはいずれも変更しない。production・`workflow_dispatch`・Gemini API・通常の外部収集も行わない。
