...
- **残作業:** ユーザーによるPR最終確認、Ready化・merge、merge後最初の通常production runでのArchive footer更新（過去記事・daily JSONは維持）の客観確認、後続BL-031（全取得元規約監査、`SECURITY_REQUIREMENTS.md`／`SECURITY_OPERATIONS.md`整合化を含む）・BL-032（取得元別ポリシー実装）・BL-009（About／出典／免責／訂正窓口）の着手判断。
