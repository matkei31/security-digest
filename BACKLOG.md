...
- **実装証跡(tranche 3e):** 選択scope(`test_security_requirements.py`の`Bl034Round2ReviewCorrectionsTest`・`Bl034ImplementationAcceptanceTest`・`Bl034CloseoutTest`・`StatusSecurityRequirementsSourceOfTruthTest`、4 classes・36 methods・143 assertions、custom assertion helperなし、fingerprint duplicate 17 group)へ、tranche 3b〜3dで確立した基準(document context・meaning-preserving reflow/rewordに対する脆弱性で分類する、raw negative multi-token substringはCとする、structural regexは reformatting-immuneなためBとする、`document_test_utils.normalize_markdown_prose`で既に正規化済みの比較はBとする、exact fact とwrapper proseを1つのliteralに束ねたものはCとする、単発のexact machine-readable valueは反復patternやshared helper consolidation価値を持たない限りA化せずB／Dとする)を適用した。PR #85 round 2で再確立された基準(Category Aは複数methodにわたる反復patternとshared helper consolidation価値が必須、fingerprint duplicateのみでは不十分)に基づき、17 fingerprint dupe group(同一assertionが複数roundの記録methodで再確認のため反復)を個別に検討したが、いずれも「同じ歴史的事実を異なるcheckpointで独立に再確認する」という性質であり(round毎に異なる文脈で同じVersionやStatus値を確認する等)、helper化すると各roundの独立した意味が失われるため、Category A該当なしと判断した(test_status.pyと同じくA 0)。分類結果: **test_security_requirements.py: A 0／B 49／C 62／D 32(total 143)**。combined: **A 18／B 176／C 226／D 103(total 523)**。`document_test_classification.json`を`scope: [test_custom_domain.py, test_ui_spec.py, test_status.py, test_security_requirements.py]`・523 entries(既存380 entriesはbyte-identicalに無変更、新規143 entriesをsource順で追記)・534行へ拡張し、`document_test_inventory.py --check`成功(combined A=18 B=176 C=226 D=103、per-file test_custom_domain.py=97/test_ui_spec.py=185/test_status.py=98/test_security_requirements.py=143、unclassified/stale/fingerprint mismatch いずれも0)を確認した。`test_document_test_classification.py`を4-file scopeへ一般化し(`SECURITY_REQUIREMENTS_SOURCE_FILE`定数・`SECURITY_REQUIREMENTS_EXPECTED_CLASSES`・`SECURITY_REQUIREMENTS_EXPECTED_A/C/D_IDS`(A 0件・C 62件・D 32件)を追加、既存3-fileの期待値は無変更のまま)、combined exact membership guardを4-file unionへ拡張し、scope shrinkage mutation testへ`class-shrink-within-security-requirements`・`file-shrink-drop-security-requirements-entirely`の2シナリオを追加し、file order swap mutation testを4-fileの全swap pair(6 pairs、itertools.combinations)へ拡張した。既存の削除・追加・fingerprint改変・category-action不整合の4種のmutation testは`SCOPE_FILES`タプル経由で自動的に4-fileへ対象拡大した。genuine multi-target entry 1件(`test_google_verification_txt_value_is_not_present_anywhere::assert-01`、BACKLOG.md/STATUS.md/DECISIONS.md/SECURITY_REQUIREMENTS.mdの4文書を1つのfor-loop assertionで検証)を`EXPECTED_MULTI_TARGETS`へ追加した。mutation-style verification(commitしない一時変更、対象document `SECURITY_REQUIREMENTS.md`・`BACKLOG.md`のみ、検証後にすべて復元)により、Category C該当代表例(SR-045の「implemented, user-accepted, and merged」を意味を変えず「implemented, accepted by the user, and merged」へreword)が実際に`assertIn`を壊すこと、Category B該当代表例(SR-046 Trigger cellのload-bearingな`BL-032`再導入)が正しく検出される一方、無関係列(Rationale)のharmless rewordでは影響を受けないこと、Category D該当代表例(BACKLOG.mdの計測開始日`2026-08-03`の日付変更)が正しく検出されることを確認した。全mutationは検証後に復元し、working tree差分が意図したファイルだけであることを確認した。tranche 3d closeout同期(最終受入・merge・Pages記録)はこのPRの一部として同時にsyncした。Category C該当62件はこのPRでのsource変換対象ではない(action `refactor_later`をmanifestへ記録するのみ、変換は別途ユーザー承認後の残作業)。
- **残作業:** Category C conversion(tranche 3bの41件・tranche 3cの84件・tranche 3dの39件・tranche 3eの62件、方針・実施は別途ユーザー承認後)、CJK prose normalizationのnuance(space-collapsingではなくspace-strippingが必要、round 1レビューで確認)、tranche 3eのDraft PR独立レビュー・最終受入・Ready化・merge、broad inventory全体(約1593件)のfull per-assertion classification残り(既存4-file scope・pure document/static-contract 5 candidate中2 file未着手 -- `test_pr_ci_workflow.py`27件・`test_workflow_action_pinning.py`23件 -- および`test_source_usage_policy.py`(単体177件でsafe scopeなし)・`test_security_operations.py`残34件(170-136)・`test_security_requirements.py`残605件(748-143))、Category A(tranche 3b 8件・tranche 3c 10件)のhelper consolidation要否判断(tranche 3d・3eはCategory A該当なしと確定済み)、残candidateのmutation-style verification、BL-038全体の最終受入。tranche 1・2・3a・3b・3c・3dは受入済み、tranche 3eは実装中(独立レビュー・最終受入は未実施)であり、BL-038全体の最終受入は上記残作業が完了するまで行わない。
